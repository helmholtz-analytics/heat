...
The vector has been repeated for every row of the left-hand side matrix. A full description of broadcasting rules can be found in [NumPy's manual](https://numpy.org/devdocs/user/theory.broadcasting.html). While talking about it, Heat is designed as seamless drop-in replacement for NumPy. There still might be cases, e.g. working with native Python code, when you want to convert a `DNDarray` to an `ndarray` instead.
