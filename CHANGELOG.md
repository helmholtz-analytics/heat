...
### `heat.core.io` refactor

- `heat/core/io.py` is now a package, `heat/core/io/`, with one module per file format
  (`csv.py`, `hdf5.py`, `netcdf.py`, `npy.py`, `zarr.py`), generic `load`/`save` in
  `io.py`, cross-format helpers in `utils.py`, and a format registry in `_registry.py`.
  Formats register their extensions and load/save functions, so `load`/`save` dispatch
  by lookup instead of a hand-maintained `if`/`elif` chain.
- Optional-dependency detection moved to `heat.core._config`. `import heat` no longer
  imports `h5py`, `netCDF4`, `zarr` or `pandas`; each is resolved on first use.
- New `ht.io.supports(name)` and `ht.io.available_formats()`, driven by the registry.
  `supports_hdf5()`, `supports_netcdf()`, `supports_zarr()` and `supports_pandas()`
  still work but now emit a `DeprecationWarning`.
- `ht.supports_pandas()` now exists; it was the only `supports_*` function missing from
  `__all__`.

### Bug fixes

- `DNDarray.save_hdf5` accepted a `dtype` argument and silently dropped it.
- `load_zarr` materialized the entire requested region on every process before taking
  its own chunk, defeating the distributed read. Each process now reads only its own
  region.
- `save_hdf5`'s serial fallback had no error propagation, so a failure on any rank left
  the others blocked forever. Both HDF5 and netCDF now share one implementation.
- `save_zarr` used the global `MPI_WORLD` rather than the array's own communicator.
- `load_npy_from_path` and `load_csv_from_folder` did not sort the directory listing, so
  the concatenation order could differ between processes.
- A broken-but-installed optional dependency no longer makes `import heat` fail.

### Breaking changes

- `load_zarr` now defaults to `split=None`, matching `load_hdf5` and `load_netcdf`; it
  previously defaulted to `split=0`. Loading with a wildcard `variable` now requires an
  explicit `split`.
- `DNDarray.save_hdf5` and `DNDarray.save_netcdf` take `(*args, **kwargs)` and forward
  them, rather than declaring their arguments explicitly.
- `heat/core/_config.py` gained an `__all__`, so `ht.os`, `ht.re`, `ht.Enum`,
  `ht.mpi4py`, `ht.platform`, `ht.subprocess` and `ht.dataclasses` are no longer
  exported. They were never intentional API.

### Additions

- `DNDarray.save_csv` and `DNDarray.save_zarr`, which HDF5 and netCDF already had.
- The `docs` extra now installs the io dependencies, so the format functions appear in
