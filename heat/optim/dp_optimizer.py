...
class DASOLayers:
    ...
    def __prepare_mpi_groups(self, use_mpi_groups: bool):
        ...
        # to be equal) and each group has a single unique GPU from each node
        # this will set the reduced_comms and reduced_ranks
        rank = self.comm.rank
    ...
    def _gs_send_params(self, current_comm: MPICommunication, batches_to_wait: int) -> None:
        ...
        sndparams = torch.zeros(
            ...
            device=self.device,
            dtype=self.sending_buffer_dtype if cast else None,
        )
