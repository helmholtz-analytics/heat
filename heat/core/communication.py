...
from ._config import HAVE_MPI, GPU_AWARE_MPI
...
class NonCommunication(Communication):
    """
    Dummy Class encapsulating no MPI Communication
    """

    @staticmethod
    def is_distributed() -> bool:
        """
        No communication is done on this communicator.
        """
        return False
...
if HAVE_MPI:
    ...
    class MPICommunication(Communication):
        ...
        def _handle_large_count(
            cls, mpi_type: MPI.Datatype, elements: int
        ...
        def mpi_type_and_elements_of(
            ...
            for i in range(len(shape) - 1, -1, -1):
                mpi_type = mpi_type.Create_vector(shape[i], 1, strides[i]).Create_resized(
                    0, offsets[i]
                )
                mpi_type.Commit()
        ...
        def alltoall_sendbuffer(
            ...
            mpi_short_type = mpi_type.Create_vector(
                send_elements[-1], 1, strides[1]
            ).Create_resized(0, offsets[1])
        ...
        def __derived_op(
            ...
            def op(sendbuf: MPI.memory, recvbuf: MPI.memory, datatype):
                ...
                send_tensor = torch.as_strided(
                    torch.frombuffer(send_arr, dtype=dtype, count=count, offset=offset),
                    shape,
                    stride,
                )
                recv_tensor = torch.as_strided(
                    torch.frombuffer(recv_arr, dtype=dtype, count=count, offset=offset),
                    shape,
                    stride,
                )
        ...
        def __reduce_like(
            ...
            **kwargs: Any
            ...
            if isinstance(recvbuf, torch.Tensor):
                ...
                recvbuf: Tuple[MPI.memory, int, MPI.Datatype] = self.as_buffer(
                    rbuf, is_contiguous=True
                )
        ...
        def Reduce(
            ...
            root: int
                Rank of the root process
            """
            ret, sbuf, rbuf, buf = self.__reduce_like(
                self.handle.Reduce, sendbuf, recvbuf, op, root
            )
            if (
                buf is not None
                and isinstance(buf, torch.Tensor)
            ...
            for idx, subarray_params in enumerate(subarray_params_list):
                ...
                if np.all(np.array(subsizes) > 0):
                    if (
                        ...
                        send_counts[idx] = (
                            1 if len(subsizes) == 2 and stride[-1] == 1 else subsizes[0]
                        )
                else:
                    send_counts[idx] = 0
        ...
        def _create_recursive_vectortype(
            ...
            displacement = (
                sum([x * y for x, y in zip(tensor_stride, start)]) * datatype.Get_extent()[1]
            )
        ...
        def Ialltoallv(
            ...
            """
            return MPIRequest(
                *self.__alltoall_like(
                    self.handle.Ialltoallv, sendbuf, recvbuf, send_axis, recv_axis
                )
            )
        ...
        def __gather_like(
            ...
            **kwargs
            ...
            sbuf = (
                self._moveToCompDevice(sendbuf, func)
                if isinstance(sendbuf, torch.Tensor)
                else sendbuf
            )
            rbuf = (
                self._moveToCompDevice(recvbuf, func)
                if isinstance(recvbuf, torch.Tensor)
                else recvbuf
            )
