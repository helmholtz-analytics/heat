...
def concatenate(arrays, axis=0):
    ...
    if s0 is None and s1 is None:
        return factories.array(torch.cat(arrays, dim=axis))
    elif s0 == s1:
        if s0 == axis:  # case 1
        ...
        if s0 == axis:
            ...
            for en, sz in enumerate(chunk_map[:-1, axis]):
                if arr1.comm.rank == en:
                    lcl_slice = [None] * arr1.numdims
                    lcl_slice[axis] = slice(arr1.lshape[axis] - chunk_map[-1, axis].item(), arr1.lshape[axis], 1)
            # for en, sl in enumerate(chunk_map[:, axis]):
            #     if arr1.comm.rank == en:
            #         # print(en, sl)
            #         # make slice to modify arr1 on each process
            #         lcl_slice = [None] * arr1.numdims
            #         lcl_slice[axis] = slice(arr1.lshape[axis]-sl.item(), arr1.lshape[axis], 1)
            #         print(lcl_slice)
            #         arr1._DNDarray__array = arr1._DNDarray__array[lcl_slice].clone()
            # print(arr1.lshape)

            # print(torch.cat((arr0._DNDarray__array, arr1._DNDarray__array), dim=axis))

        # _, _, arb_slice = arr0.comm.chunk(arr1.shape, arr1.split)
