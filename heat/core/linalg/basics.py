...
def cross(
    ...
    axis : int
        Axis that defines the vectors for which to compute the cross product. Overrides `axisa`,
        `axisb` and `axisc`. Default: -1

    Raises
    ------
    ValueError
        If the two input arrays don't match in shape, split, device, or comm. If the vectors are
            ...
            if pr == b.comm.size - 1:
                ...
                if a_rem_locs0.nelement() != 0:
                    if r_loc is not None:
                        ...
                        st1 = index_map[pr, 1, 1, 0].item() if split_01_flag else None
                        sp1 = index_map[pr, 1, 1, 1].item() if split_01_flag else None
...
def __tri_op(m: DNDarray, k: int, op: Callable) -> DNDarray:
    """
    Generic implementation of triangle operations on a ``DNDarray``. It takes care of input
    sanitation and non-standard broadcast behavior of the 2D triangle-operators.
