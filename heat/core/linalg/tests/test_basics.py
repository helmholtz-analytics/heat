...
class TestLinalgBasics(TestCase):
    def test_dot(self):
        ...
        a1d = ht.array(data1d, dtype=ht.float32, split=0, device=self.ht_device)
        b1d = ht.array(data1d, dtype=ht.float32, split=0, device=self.ht_device)
        ...
        a1d = ht.array(data1d, dtype=ht.float32, split=None, device=self.ht_device)
        b1d = ht.array(data1d, dtype=ht.float32, split=0, device=self.ht_device)
        self.assertEqual(ht.dot(a1d, b1d), np.dot(data1d, data1d))

        a1d = ht.array(data1d, dtype=ht.float32, split=None, device=self.ht_device)
        b1d = ht.array(data1d, dtype=ht.float32, split=None, device=self.ht_device)
        self.assertEqual(ht.dot(a1d, b1d), np.dot(data1d, data1d))
        ...
        a2d = ht.array(data2d, split=1, device=self.ht_device)
        b2d = ht.array(data2d, split=1, device=self.ht_device)
        # 2 2D arrays,
        res = ht.dot(a2d, b2d) - ht.array(np.dot(data2d, data2d), device=self.ht_device)
        self.assertEqual(ht.equal(res, ht.zeros(res.shape, device=self.ht_device)), 1)
        ret = ht.array(data2d, split=1, device=self.ht_device)
        ht.dot(a2d, b2d, out=ret)

        res = ret - ht.array(np.dot(data2d, data2d), device=self.ht_device)
        self.assertEqual(ht.equal(res, ht.zeros(res.shape, device=self.ht_device)), 1)

        const1 = 5
        const2 = 6
        # a is const
        res = ht.dot(const1, b2d) - ht.array(np.dot(const1, data2d), device=self.ht_device)
        ret = 0
        ht.dot(const1, b2d, out=ret)
        self.assertEqual(ht.equal(res, ht.zeros(res.shape, device=self.ht_device)), 1)

        # b is const
        res = ht.dot(a2d, const2) - ht.array(np.dot(data2d, const2), device=self.ht_device)
        self.assertEqual(ht.equal(res, ht.zeros(res.shape, device=self.ht_device)), 1)
        # a and b and const
        self.assertEqual(ht.dot(const2, const1), 5 * 6)

        with self.assertRaises(NotImplementedError):
            ht.dot(ht.array(data3d, device=self.ht_device), ht.array(data1d, device=self.ht_device))

    def test_matmul(self):
        with self.assertRaises(ValueError):
            ht.matmul(
                ht.ones((25, 25), device=self.ht_device), ht.ones((42, 42), device=self.ht_device)
            )

        # cases to test:
        n, m = 21, 31
        j, k = m, 45
        a_torch = torch.ones((n, m), device=self.torch_device)
        a_torch[0] = torch.arange(1, m + 1, device=self.torch_device)
        a_torch[:, -1] = torch.arange(1, n + 1, device=self.torch_device)
        b_torch = torch.ones((j, k), device=self.torch_device)
        b_torch[0] = torch.arange(1, k + 1, device=self.torch_device)
        b_torch[:, 0] = torch.arange(1, j + 1, device=self.torch_device)

        # splits None None
        a = ht.ones((n, m), split=None, device=self.ht_device)
        b = ht.ones((j, k), split=None, device=self.ht_device)
        a[0] = ht.arange(1, m + 1, device=self.ht_device)
        a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
        b[0] = ht.arange(1, k + 1, device=self.ht_device)
        b[:, 0] = ht.arange(1, j + 1, device=self.ht_device)
        ret00 = ht.matmul(a, b)

        self.assertEqual(ht.all(ret00 == ht.array(a_torch @ b_torch, device=self.ht_device)), 1)
        ...
        if a.comm.size > 1:
            # splits 00
            a = ht.ones((n, m), split=0, dtype=ht.float64, device=self.ht_device)
            b = ht.ones((j, k), split=0, device=self.ht_device)
            a[0] = ht.arange(1, m + 1, device=self.ht_device)
            a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
            b[0] = ht.arange(1, k + 1, device=self.ht_device)
            b[:, 0] = ht.arange(1, j + 1, device=self.ht_device)
            ...
            ret_comp00 = ht.array(a_torch @ b_torch, split=0, device=self.ht_device)
            ...
            # splits 00 (numpy)
            a = ht.array(np.ones((n, m)), split=0, device=self.ht_device)
            b = ht.array(np.ones((j, k)), split=0, device=self.ht_device)
            a[0] = ht.arange(1, m + 1, device=self.ht_device)
            a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
            b[0] = ht.arange(1, k + 1, device=self.ht_device)
            b[:, 0] = ht.arange(1, j + 1, device=self.ht_device)
            ...
            # splits 01
            a = ht.ones((n, m), split=0, device=self.ht_device)
            b = ht.ones((j, k), split=1, dtype=ht.float64, device=self.ht_device)
            a[0] = ht.arange(1, m + 1, device=self.ht_device)
            a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
            b[0] = ht.arange(1, k + 1, device=self.ht_device)
            b[:, 0] = ht.arange(1, j + 1, device=self.ht_device)
            ...
            ret_comp01 = ht.array(a_torch @ b_torch, split=0, device=self.ht_device)
            ...
            # splits 10
            a = ht.ones((n, m), split=1, device=self.ht_device)
            b = ht.ones((j, k), split=0, device=self.ht_device)
            a[0] = ht.arange(1, m + 1, device=self.ht_device)
            a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
            b[0] = ht.arange(1, k + 1, device=self.ht_device)
            ...
            ret_comp10 = ht.array(a_torch @ b_torch, split=1, device=self.ht_device)
            ...
            # splits 11
            a = ht.ones((n, m), split=1, device=self.ht_device)
            b = ht.ones((j, k), split=1, device=self.ht_device)
            a[0] = ht.arange(1, m + 1, device=self.ht_device)
            a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
            b[0] = ht.arange(1, k + 1, device=self.ht_device)
            b[:, 0] = ht.arange(1, j + 1, device=self.ht_device)
            ...
            ret_comp11 = ht.array(a_torch @ b_torch, split=1, device=self.ht_device)
            ...
            # splits 11 (torch)
            a = ht.array(
                torch.ones((n, m), device=self.torch_device), split=1, device=self.ht_device
            )
            b = ht.array(
                torch.ones((j, k), device=self.torch_device), split=1, device=self.ht_device
            )
            a[0] = ht.arange(1, m + 1, device=self.ht_device)
            a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
            b[0] = ht.arange(1, k + 1, device=self.ht_device)
            ...
            ret_comp0 = ht.array(a_torch @ b_torch, split=0, device=self.ht_device)
            ...
            ret_comp1 = ht.array(a_torch @ b_torch, split=1, device=self.ht_device)
            ...
            ret_comp = ht.array(a_torch @ b_torch, split=0, device=self.ht_device)
            ...
            ret_comp = ht.array(a_torch @ b_torch, split=1, device=self.ht_device)
            self.assertTrue(ht.equal(ret00, ret_comp))
            self.assertIsInstance(ret00, ht.DNDarray)
            self.assertEqual(ret00.shape, (n, k))
            self.assertEqual(ret00.dtype, ht.float)
            ...
            # vector matrix mult:
            # a -> vector
            a_torch = torch.ones((m), device=self.torch_device)
            b_torch = torch.ones((j, k), device=self.torch_device)
            b_torch[0] = torch.arange(1, k + 1, device=self.torch_device)
            b_torch[:, 0] = torch.arange(1, j + 1, device=self.torch_device)
            # splits None None
            a = ht.ones((m), split=None, device=self.ht_device)
            b = ht.ones((j, k), split=None, device=self.ht_device)
            b[0] = ht.arange(1, k + 1, device=self.ht_device)
            b[:, 0] = ht.arange(1, j + 1, device=self.ht_device)
            ret00 = ht.matmul(a, b)

            ret_comp = ht.array(a_torch @ b_torch, split=None, device=self.ht_device)
            self.assertTrue(ht.equal(ret00, ret_comp))
            self.assertIsInstance(ret00, ht.DNDarray)
            ...
            # splits 0 0
            a = ht.ones((m), split=0, device=self.ht_device)
            b = ht.ones((j, k), split=0, device=self.ht_device)
            b[0] = ht.arange(1, k + 1, device=self.ht_device)
            b[:, 0] = ht.arange(1, j + 1, device=self.ht_device)
            ret00 = ht.matmul(a, b)
            ...
            a_torch[0] = torch.arange(1, m + 1, device=self.torch_device)
            a_torch[:, -1] = torch.arange(1, n + 1, device=self.torch_device)
            b_torch = torch.ones((j), device=self.torch_device)
            # splits None None
            a = ht.ones((n, m), split=None, device=self.ht_device)
            b = ht.ones((j), split=None, device=self.ht_device)
            a[0] = ht.arange(1, m + 1, device=self.ht_device)
            a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
            ret00 = ht.matmul(a, b)
            ...
            # splits None 0
            a = ht.ones((n, m), split=None, device=self.ht_device)
            b = ht.ones((j), split=0, device=self.ht_device)
            a[0] = ht.arange(1, m + 1, device=self.ht_device)
            a[:, -1] = ht.arange(1, n + 1, device=self.ht_device)
            ret00 = ht.matmul(a, b)
    ...
    def test_transpose(self):
        ...
        vector = ht.arange(10, device=self.ht_device)
        vector_t = vector.T
        ...
        simple_matrix = ht.zeros((2, 4), device=self.ht_device)
        ...
        array_4d = ht.zeros((2, 3, 4, 5), device=self.ht_device)
        ...
        vector_split = ht.arange(10, split=0, device=self.ht_device)
        ...
        matrix_split = ht.ones((10, 20), split=1, device=self.ht_device)
        ...
        array_4d_split = ht.ones((3, 4, 5, 6), split=3, device=self.ht_device)
        ...
        with self.assertRaises(TypeError):
            ht.transpose(1)
        with self.assertRaises(ValueError):
            ht.transpose(ht.zeros((2, 3), device=self.ht_device), axes=1.0)
        with self.assertRaises(ValueError):
            ht.transpose(ht.zeros((2, 3), device=self.ht_device), axes=(-1,))
        with self.assertRaises(TypeError):
            ht.zeros((2, 3), device=self.ht_device).transpose(axes="01")
        with self.assertRaises(TypeError):
            ht.zeros((2, 3), device=self.ht_device).transpose(axes=(0, 1.0))
        with self.assertRaises((ValueError, IndexError)):
            ht.zeros((2, 3), device=self.ht_device).transpose(axes=(0, 3))

    def test_tril(self):
        local_ones = ht.ones((5,), device=self.ht_device)

        # 1D case, no offset, data is not split, module-level call
        result = ht.tril(local_ones)
        comparison = torch.ones((5, 5), device=self.torch_device).tril()
        ...
        comparison = torch.ones((5, 5), device=self.torch_device).tril(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (5, 5))
        self.assertEqual(result.lshape, (5, 5))
        self.assertEqual(result.split, None)
        ...
        local_ones = ht.ones((4, 5), device=self.ht_device)

        # 2D case, no offset, data is not split, method
        result = local_ones.tril()
        comparison = torch.ones((4, 5), device=self.torch_device).tril()
        self.assertIsInstance(result, ht.DNDarray)
        ...
        comparison = torch.ones((4, 5), device=self.torch_device).tril(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (4, 5))
        self.assertEqual(result.lshape, (4, 5))
        self.assertEqual(result.split, None)
        ...
        local_ones = ht.ones((3, 4, 5, 6), device=self.ht_device)

        # 2D+ case, no offset, data is not split, module-level call
        result = local_ones.tril()
        comparison = torch.ones((5, 6), device=self.torch_device).tril()
        self.assertIsInstance(result, ht.DNDarray)
        ...
        result = local_ones.tril(k=2)
        comparison = torch.ones((5, 6), device=self.torch_device).tril(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (3, 4, 5, 6))
        self.assertEqual(result.lshape, (3, 4, 5, 6))
        self.assertEqual(result.split, None)
        ...
        distributed_ones = ht.ones((5,), split=0, device=self.ht_device)
        ...
        distributed_ones = ht.ones((4, 5), split=0, device=self.ht_device)
        ...
        distributed_ones = ht.ones((4, 5), split=1, device=self.ht_device)
    ...
    def test_triu(self):
        ...
        comparison = torch.ones((5, 5), device=self.torch_device).triu()
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (5, 5))
        self.assertEqual(result.lshape, (5, 5))
        self.assertEqual(result.split, None)
        ...
        comparison = torch.ones((5, 5), device=self.torch_device).triu(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (5, 5))
        self.assertEqual(result.lshape, (5, 5))
        self.assertEqual(result.split, None)
        ...
        comparison = torch.ones((4, 5), device=self.torch_device).triu()
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (4, 5))
        self.assertEqual(result.lshape, (4, 5))
        self.assertEqual(result.split, None)
        ...
        comparison = torch.ones((4, 5), device=self.torch_device).triu(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (4, 5))
        self.assertEqual(result.lshape, (4, 5))
        self.assertEqual(result.split, None)
        ...
        # 2D+ case, no offset, data is not split, module-level call
        result = local_ones.triu()
        comparison = torch.ones((5, 6), device=self.torch_device).triu()
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (3, 4, 5, 6))
        self.assertEqual(result.lshape, (3, 4, 5, 6))
        self.assertEqual(result.split, None)
        ...
        # 2D+ case, positive offset, data is not split, module-level call
        result = local_ones.triu(k=2)
        comparison = torch.ones((5, 6), device=self.torch_device).triu(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (3, 4, 5, 6))
        self.assertEqual(result.lshape, (3, 4, 5, 6))
        self.assertEqual(result.split, None)
