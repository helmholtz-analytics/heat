...
class TestFactories(BasicTest):
    ...
    def test_arange(self):
        ...
        one_arg_arange_float = ht.arange(10.0, device=self.ht_device)
        ...
        two_arg_arange_int = ht.arange(0, 10, device=self.ht_device)
        ...
        two_arg_arange_float = ht.arange(0.0, 10, device=self.ht_device)
        ...
        three_arg_arange_int = ht.arange(0, 10, 2, device=self.ht_device)
        ...
        three_arg_arange_float = ht.arange(0, 10, 2.0, device=self.ht_device)
        ...
        # testing splitting
        three_arg_arange_dtype_float32 = ht.arange(0, 10, 2.0, split=0, device=self.ht_device)
        ...
        three_arg_arange_dtype_short = ht.arange(
            0, 10, 2.0, dtype=torch.int16, device=self.ht_device
        )
        ...
        three_arg_arange_dtype_float64 = ht.arange(
            0, 10, 2, dtype=torch.float64, device=self.ht_device
        )
        ...
        with self.assertRaises(ValueError):
            ht.arange(-5, 3, split=1, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.arange(device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.arange(1, 2, 3, 4, device=self.ht_device)

    def test_array(self):
        # basic array function, unsplit data
        unsplit_data = [[1, 2, 3], [4, 5, 6]]
        a = ht.array(unsplit_data, device=self.ht_device)
        ...
        self.assertTrue(
            (a._DNDarray__array == torch.tensor(unsplit_data, device=self.torch_device)).all()
        )

        # basic array function, unsplit data, different datatype
        tuple_data = ((0, 0), (1, 1))
        b = ht.array(tuple_data, dtype=ht.int8, device=self.ht_device)
        ...
        self.assertTrue(
            (
                b._DNDarray__array
                == torch.tensor(tuple_data, dtype=torch.int8, device=self.torch_device)
            ).all()
        )

        # basic array function, unsplit data, no copy
        torch_tensor = torch.tensor([6, 5, 4, 3, 2, 1], device=self.torch_device)
        c = ht.array(torch_tensor, copy=False, device=self.ht_device)
        ...
        d = ht.array(vector_data, ndmin=3, device=self.ht_device)
        ...
        self.assertTrue(
            (
                d._DNDarray__array
                == torch.tensor(vector_data, device=self.torch_device).reshape(-1, 1, 1)
            ).all()
        )
        ...
        tensor_2d = ht.array(
            [[1.0, 2.0, 3.0], [1.0, 2.0, 3.0], [1.0, 2.0, 3.0]], split=0, device=self.ht_device
        )
        ...
        self.assertTrue(
            (
                ...
                == torch.tensor([1.0, 2.0, 3.0], device=self.torch_device)
            ).all()
        )
        ...
        e = ht.array(split_data, ndmin=3, is_split=0, device=self.ht_device)
        ...
        if ht.communication.MPI_WORLD.size > 1:
            ...
            with self.assertRaises(ValueError):
                ht.array(split_data, is_split=0, device=self.ht_device)
        ...
        if ht.communication.MPI_WORLD.size > 1:
            ...
            with self.assertRaises(ValueError):
                ht.array(split_data, is_split=1, device=self.ht_device)

        # check exception on mutually exclusive split and is_split
        with self.assertRaises(ValueError):
            ht.array([[1.0, 2.0, 3.0], [1.0, 2.0, 3.0]], split=0, is_split=0, device=self.ht_device)

        e = ht.array(split_data, ndmin=-3, is_split=1, device=self.ht_device)
        ...
        # check exception on mutually exclusive split and is_split
        with self.assertRaises(ValueError):
            ht.array([[1.0, 2.0, 3.0], [1.0, 2.0, 3.0]], split=1, is_split=1, device=self.ht_device)

        # non iterable type
        with self.assertRaises(TypeError):
            ht.array(map, device=self.ht_device)
        # iterable, but unsuitable type
        with self.assertRaises(TypeError):
            ht.array("abc", device=self.ht_device)
        # unknown dtype
        with self.assertRaises(TypeError):
            ht.array((4,), dtype="a", device=self.ht_device)
        # invalid ndmin
        with self.assertRaises(TypeError):
            ht.array((4,), ndmin=3.0, device=self.ht_device)
        # invalid split axis type
        with self.assertRaises(TypeError):
            ht.array((4,), split="a", device=self.ht_device)
        # invalid split axis value
        with self.assertRaises(ValueError):
            ht.array((4,), split=3, device=self.ht_device)
        # invalid communicator
        with self.assertRaises(TypeError):
            ht.array((4,), comm={}, device=self.ht_device)

    def test_empty(self):
        # scalar input
        simple_empty_float = ht.empty(3, device=self.ht_device)
        ...
        # different data type
        simple_empty_uint = ht.empty(5, dtype=ht.bool, device=self.ht_device)
        ...
        # multi-dimensional
        elaborate_empty_int = ht.empty((2, 3), dtype=ht.int32, device=self.ht_device)
        ...
        # split axis
        elaborate_empty_split = ht.empty((6, 4), dtype=ht.int32, split=0, device=self.ht_device)
        ...
        # exceptions
        with self.assertRaises(TypeError):
            ht.empty("(2, 3,)", dtype=ht.float64, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.empty((-1, 3), dtype=ht.float64, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.empty((2, 3), dtype=ht.float64, split="axis", device=self.ht_device)

    def test_empty_like(self):
        # scalar
        like_int = ht.empty_like(3, device=self.ht_device)
        ...
        # sequence
        like_str = ht.empty_like("abc", device=self.ht_device)
        ...
        # elaborate tensor
        ones = ht.ones((2, 3), dtype=ht.uint8, device=self.ht_device)
        like_ones = ht.empty_like(ones, device=self.ht_device)
        ...
        ones_split = ht.ones((2, 3), dtype=ht.uint8, split=0, device=self.ht_device)
        like_ones_split = ht.empty_like(ones_split, device=self.ht_device)
        ...
        # exceptions
        with self.assertRaises(TypeError):
            ht.empty_like(ones, dtype="abc", device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.empty_like(ones, split="axis", device=self.ht_device)
    ...
    def test_eye(self):
        ...
        shape = 5
        eye = ht.eye(shape, dtype=ht.uint8, split=1, device=self.ht_device)
        ...
        shape = (10, 20)
        eye = ht.eye(shape, dtype=ht.float32, device=self.ht_device)
        self.assertIsInstance(eye, ht.DNDarray)
        ...
        shape = (10,)
        eye = ht.eye(shape, dtype=ht.int32, split=0, device=self.ht_device)
        self.assertIsInstance(eye, ht.DNDarray)
        ...
        shape = (11, 30)
        eye = ht.eye(shape, split=1, dtype=ht.float32, device=self.ht_device)
        self.assertIsInstance(eye, ht.DNDarray)
        self.assertEqual(eye.dtype, ht.float32)
        self.assertEqual(eye.shape, shape)
        self.assertEqual(eye.split, 1)
    ...
    def test_full(self):
        # simple tensor
        data = ht.full((10, 2), 4, device=self.ht_device)
        ...
        self.assertTrue(ht.allclose(data, ht.float32(4.0, device=self.ht_device)))

        # non-standard dtype tensor
        data = ht.full((10, 2), 4, dtype=ht.int32, device=self.ht_device)
        self.assertIsInstance(data, ht.DNDarray)
        self.assertEqual(data.shape, (10, 2))
        self.assertEqual(data.lshape, (10, 2))
        ...
        self.assertEqual(data.split, None)
        self.assertTrue(ht.allclose(data, ht.int32(4, device=self.ht_device)))

        # split tensor
        data = ht.full((10, 2), 4, split=0, device=self.ht_device)
        self.assertIsInstance(data, ht.DNDarray)
        self.assertEqual(data.shape, (10, 2))
        ...
        # exceptions
        with self.assertRaises(TypeError):
            ht.full("(2, 3,)", 4, dtype=ht.float64, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.full((-1, 3), 2, dtype=ht.float64, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.full((2, 3), dtype=ht.float64, split="axis", device=self.ht_device)
    ...
    def test_full_like(self):
        ...
        self.assertTrue(ht.allclose(like_int, ht.float32(4, device=self.ht_device)))
        ...
        self.assertEqual(like_str.split, None)
        self.assertEqual(like_str.dtype, ht.float32)
        self.assertTrue(ht.allclose(like_str, ht.float32(2, device=self.ht_device)))

        # elaborate tensor
        zeros = ht.zeros((2, 3), dtype=ht.uint8, device=self.ht_device)
        ...
        self.assertTrue(ht.allclose(like_zeros, ht.float32(7, device=self.ht_device)))

        # elaborate tensor with split
        zeros_split = ht.zeros((2, 3), dtype=ht.uint8, split=0, device=self.ht_device)
        ...
        self.assertTrue(ht.allclose(like_zeros_split, ht.float32(6, device=self.ht_device)))

        # exceptions
        with self.assertRaises(TypeError):
            ht.ones_like(zeros, dtype="abc", device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.ones_like(zeros, split="axis", device=self.ht_device)

    def test_linspace(self):
        # simple linear space
        ascending = ht.linspace(-3, 5, device=self.ht_device)
        ...
        descending = ht.linspace(-5, 3, num=100, device=self.ht_device)
        ...
        # split linear space
        split = ht.linspace(-5, 3, num=70, split=0, device=self.ht_device)
        ...
        # with casted type
        casted = ht.linspace(-5, 3, num=70, dtype=ht.uint8, split=0, device=self.ht_device)
        ...
        # retstep test
        result = ht.linspace(
            -5, 3, num=70, retstep=True, dtype=ht.uint8, split=0, device=self.ht_device
        )
        ...
        # exceptions
        with self.assertRaises(ValueError):
            ht.linspace(-5, 3, split=1, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.linspace(-5, 3, num=-1, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.linspace(-5, 3, num=0, device=self.ht_device)

    def test_logspace(self):
        # simple log space
        ascending = ht.logspace(-3, 5, device=self.ht_device)
        self.assertIsInstance(ascending, ht.DNDarray)
        self.assertEqual(ascending.shape, (50,))
        self.assertLessEqual(ascending.lshape[0], 50)
        self.assertEqual(ascending.dtype, ht.float32)
        ...
        descending = ht.logspace(-5, 3, num=100, device=self.ht_device)
        self.assertIsInstance(descending, ht.DNDarray)
        self.assertEqual(descending.shape, (100,))
        self.assertLessEqual(descending.lshape[0], 100)
        self.assertEqual(descending.dtype, ht.float32)
        ...
        # split log space
        split = ht.logspace(-5, 3, num=70, split=0, device=self.ht_device)
        self.assertIsInstance(split, ht.DNDarray)
        self.assertEqual(split.shape, (70,))
        self.assertLessEqual(split.lshape[0], 70)
        self.assertEqual(split.dtype, ht.float32)
        ...
        # with casted type
        casted = ht.logspace(-5, 3, num=70, dtype=ht.uint8, split=0, device=self.ht_device)
        self.assertIsInstance(casted, ht.DNDarray)
        self.assertEqual(casted.shape, (70,))
        self.assertLessEqual(casted.lshape[0], 70)
        self.assertEqual(casted.dtype, ht.uint8)
        ...
        # base test
        base = ht.logspace(-5, 3, num=70, base=2.0, device=self.ht_device)
        ...
        # exceptions
        with self.assertRaises(ValueError):
            ht.logspace(-5, 3, split=1, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.logspace(-5, 3, num=-1, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.logspace(-5, 3, num=0, device=self.ht_device)

    def test_ones(self):
        # scalar input
        simple_ones_float = ht.ones(3, device=self.ht_device)
        ...
        # different data type
        simple_ones_uint = ht.ones(5, dtype=ht.bool, device=self.ht_device)
        ...
        # multi-dimensional
        elaborate_ones_int = ht.ones((2, 3), dtype=ht.int32, device=self.ht_device)
        ...
        # split axis
        elaborate_ones_split = ht.ones((6, 4), dtype=ht.int32, split=0, device=self.ht_device)
        ...
        # exceptions
        with self.assertRaises(TypeError):
            ht.ones("(2, 3,)", dtype=ht.float64, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.ones((-1, 3), dtype=ht.float64, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.ones((2, 3), dtype=ht.float64, split="axis", device=self.ht_device)

    def test_ones_like(self):
        # scalar
        like_int = ht.ones_like(3, device=self.ht_device)
        self.assertIsInstance(like_int, ht.DNDarray)
        self.assertEqual(like_int.shape, (1,))
        self.assertEqual(like_int.lshape, (1,))
        self.assertEqual(like_int.split, None)
        ...
        # sequence
        like_str = ht.ones_like("abc", device=self.ht_device)
        self.assertIsInstance(like_str, ht.DNDarray)
        self.assertEqual(like_str.shape, (3,))
        self.assertEqual(like_str.lshape, (3,))
        self.assertEqual(like_str.split, None)
        ...
        # elaborate tensor
        zeros = ht.zeros((2, 3), dtype=ht.uint8, device=self.ht_device)
        like_zeros = ht.ones_like(zeros, device=self.ht_device)
        self.assertIsInstance(like_zeros, ht.DNDarray)
        self.assertEqual(like_zeros.shape, (2, 3))
        self.assertEqual(like_zeros.lshape, (2, 3))
        self.assertEqual(like_zeros.split, None)
        ...
        # elaborate tensor with split
        zeros_split = ht.zeros((2, 3), dtype=ht.uint8, split=0, device=self.ht_device)
        like_zeros_split = ht.ones_like(zeros_split, device=self.ht_device)
        self.assertIsInstance(like_zeros_split, ht.DNDarray)
        self.assertEqual(like_zeros_split.shape, (2, 3))
        self.assertLessEqual(like_zeros_split.lshape[0], 2)
        self.assertEqual(like_zeros_split.lshape[1], 3)
    ...
    def test_zeros(self):
        # scalar input
        simple_zeros_float = ht.zeros(3, device=self.ht_device)
        ...
        # different data type
        simple_zeros_uint = ht.zeros(5, dtype=ht.bool, device=self.ht_device)
        ...
        # multi-dimensional
        elaborate_zeros_int = ht.zeros((2, 3), dtype=ht.int32, device=self.ht_device)
        ...
        # split axis
        elaborate_zeros_split = ht.zeros((6, 4), dtype=ht.int32, split=0, device=self.ht_device)
        ...
        # exceptions
        with self.assertRaises(TypeError):
            ht.zeros("(2, 3,)", dtype=ht.float64, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.zeros((-1, 3), dtype=ht.float64, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.zeros((2, 3), dtype=ht.float64, split="axis", device=self.ht_device)

    def test_zeros_like(self):
        # scalar
        like_int = ht.zeros_like(3, device=self.ht_device)
        self.assertIsInstance(like_int, ht.DNDarray)
        self.assertEqual(like_int.shape, (1,))
        self.assertEqual(like_int.lshape, (1,))
        self.assertEqual(like_int.split, None)
        ...
        # sequence
        like_str = ht.zeros_like("abc", device=self.ht_device)
        self.assertIsInstance(like_str, ht.DNDarray)
        self.assertEqual(like_str.shape, (3,))
        self.assertEqual(like_str.lshape, (3,))
        self.assertEqual(like_str.split, None)
        ...
        # elaborate tensor
        ones = ht.ones((2, 3), dtype=ht.uint8, device=self.ht_device)
        like_ones = ht.zeros_like(ones, device=self.ht_device)
        self.assertIsInstance(like_ones, ht.DNDarray)
        self.assertEqual(like_ones.shape, (2, 3))
        self.assertEqual(like_ones.lshape, (2, 3))
        self.assertEqual(like_ones.split, None)
        ...
        # elaborate tensor with split
        ones_split = ht.ones((2, 3), dtype=ht.uint8, split=0, device=self.ht_device)
        like_ones_split = ht.zeros_like(ones_split, device=self.ht_device)
        self.assertIsInstance(like_ones_split, ht.DNDarray)
        self.assertEqual(like_ones_split.shape, (2, 3))
        self.assertLessEqual(like_ones_split.lshape[0], 2)
        self.assertEqual(like_ones_split.lshape[1], 3)
        ...
        # exceptions
        with self.assertRaises(TypeError):
            ht.zeros_like(ones, dtype="abc", device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.zeros_like(ones, split="axis", device=self.ht_device)
