...
class TestIO(BasicTest):
    ...
    def setUpClass(cls):
        ...
        cls.IRIS = (
            torch.from_numpy(np.loadtxt(cls.CSV_PATH, delimiter=";")).float().to(cls.torch_device)
        )
    ...
    def test_load(self):
        # HDF5
        if ht.io.supports_hdf5():
            iris = ht.load(self.HDF5_PATH, dataset="data", device=self.ht_device)
            ...
            with self.assertRaises(ValueError):
                _ = ht.load(self.HDF5_PATH, dataset=self.HDF5_DATASET, device=self.ht_device)

        # netCDF
        if ht.io.supports_netcdf():
            iris = ht.load(self.NETCDF_PATH, variable=self.NETCDF_VARIABLE, device=self.ht_device)
            self.assertIsInstance(iris, ht.DNDarray)
            # shape invariant
            self.assertEqual(iris.shape, self.IRIS.shape)
            self.assertEqual(iris._DNDarray__array.shape, self.IRIS.shape)
            ...
            with self.assertRaises(ValueError):
                _ = ht.load(self.NETCDF_PATH, variable=self.NETCDF_VARIABLE, device=self.ht_device)

    def test_load_csv(self):
        csv_file_length = 150
        csv_file_cols = 4
        first_value = torch.tensor(
            [5.1, 3.5, 1.4, 0.2], dtype=torch.float32, device=self.torch_device
        )
        tenth_value = torch.tensor(
            [4.9, 3.1, 1.5, 0.1], dtype=torch.float32, device=self.torch_device
        )

        a = ht.load_csv(self.CSV_PATH, sep=";", device=self.ht_device)
        ...
        a = ht.load_csv(self.CSV_PATH, sep=";", split=0, device=self.ht_device)
        ...
        a = ht.load_csv(
            self.CSV_PATH, sep=";", header_lines=9, dtype=ht.float32, split=0, device=self.ht_device
        )
        ...
        a = ht.load_csv(self.CSV_PATH, sep=";", split=1, device=self.ht_device)
        self.assertEqual(a.shape, (csv_file_length, csv_file_cols))
        self.assertEqual(a.lshape[0], csv_file_length)

        a = ht.load_csv(self.CSV_PATH, sep=";", split=0, device=self.ht_device)
        b = ht.load(self.CSV_PATH, sep=";", split=0, device=self.ht_device)
        self.assertTrue(ht.equal(a, b))

        # Test for csv where header is longer then the first process`s share of lines
        a = ht.load_csv(self.CSV_PATH, sep=";", header_lines=100, split=0, device=self.ht_device)
        self.assertEqual(a.shape, (50, 4))

        with self.assertRaises(TypeError):
            ht.load_csv(12314, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.load_csv(self.CSV_PATH, sep=11, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.load_csv(self.CSV_PATH, header_lines="3", sep=";", split=0, device=self.ht_device)

    def test_load_exception(self):
        # correct extension, file does not exist
        if ht.io.supports_hdf5():
            with self.assertRaises(IOError):
                ht.load("foo.h5", "data", device=self.ht_device)
        else:
            with self.assertRaises(ValueError):
                ht.load("foo.h5", "data", device=self.ht_device)
        ...
        if ht.io.supports_netcdf():
            with self.assertRaises(IOError):
                ht.load("foo.nc", "data", device=self.ht_device)
        else:
            with self.assertRaises(ValueError):
                ht.load("foo.nc", "data", device=self.ht_device)
        ...
        with self.assertRaises(ValueError):
            ht.load("iris", "data", device=self.ht_device)

    # catch-all save
    def test_save(self):
        if ht.io.supports_hdf5():
            # local range
            local_range = ht.arange(100, device=self.ht_device)
            ...
            if local_range.comm.rank == 0:
                with ht.io.h5py.File(self.HDF5_OUT_PATH, "r") as handle:
                    comparison = torch.tensor(
                        handle[self.HDF5_DATASET], dtype=torch.int32, device=self.torch_device
                    )
                self.assertTrue((local_range._DNDarray__array == comparison).all())

            # split range
            split_range = ht.arange(100, split=0, device=self.ht_device)
        ...
        if ht.io.supports_netcdf():
            ...
            if local_range.comm.rank == 0:
                with ht.io.nc.Dataset(self.NETCDF_OUT_PATH, "r") as handle:
                    comparison = torch.tensor(
                        handle[self.NETCDF_VARIABLE][:], dtype=torch.int32, device=self.torch_device
                    )
                self.assertTrue((local_range._DNDarray__array == comparison).all())
    ...
    def test_save_exception(self):
        data = ht.arange(1, device=self.ht_device)
    ...
    def test_load_hdf5(self):
        ...
        # default parameters
        iris = ht.load_hdf5(self.HDF5_PATH, self.HDF5_DATASET, device=self.ht_device)
        self.assertIsInstance(iris, ht.DNDarray)
        self.assertEqual(iris.shape, self.IRIS.shape)
        self.assertEqual(iris.dtype, ht.float32)
        self.assertEqual(iris._DNDarray__array.dtype, torch.float32)
        ...
        # positive split axis
        iris = ht.load_hdf5(self.HDF5_PATH, self.HDF5_DATASET, split=0, device=self.ht_device)
        self.assertIsInstance(iris, ht.DNDarray)
        self.assertEqual(iris.shape, self.IRIS.shape)
        self.assertEqual(iris.dtype, ht.float32)
        lshape = iris.lshape
        ...
        # different data type
        iris = ht.load_hdf5(self.HDF5_PATH, self.HDF5_DATASET, dtype=ht.int8, device=self.ht_device)
        self.assertIsInstance(iris, ht.DNDarray)
        self.assertEqual(iris.shape, self.IRIS.shape)
    ...
    def test_load_hdf5_exception(self):
        ...
        with self.assertRaises(TypeError):
            ht.load_hdf5(1, "data", device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.load_hdf5("iris.h5", 1, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.load_hdf5("iris.h5", dataset="data", split=1.0, device=self.ht_device)

        # file or dataset does not exist
        with self.assertRaises(IOError):
            ht.load_hdf5("foo.h5", dataset="data", device=self.ht_device)
        with self.assertRaises(IOError):
            ht.load_hdf5("iris.h5", dataset="foo", device=self.ht_device)
    ...
    def test_save_hdf5(self):
        ...
        # local unsplit data
        local_data = ht.arange(100, device=self.ht_device)
        ...
        split_data = ht.arange(100, split=0, device=self.ht_device)
    ...
    def test_load_netcdf(self):
        ...
        # default parameters
        iris = ht.load_netcdf(self.NETCDF_PATH, self.NETCDF_VARIABLE, device=self.ht_device)
        self.assertIsInstance(iris, ht.DNDarray)
        self.assertEqual(iris.shape, self.IRIS.shape)
        self.assertEqual(iris.dtype, ht.float32)
        self.assertEqual(iris._DNDarray__array.dtype, torch.float32)
        ...
        # positive split axis
        iris = ht.load_netcdf(
            self.NETCDF_PATH, self.NETCDF_VARIABLE, split=0, device=self.ht_device
        )
        self.assertIsInstance(iris, ht.DNDarray)
        self.assertEqual(iris.shape, self.IRIS.shape)
        self.assertEqual(iris.dtype, ht.float32)
        ...
        # different data type
        iris = ht.load_netcdf(
            self.NETCDF_PATH, self.NETCDF_VARIABLE, dtype=ht.int8, device=self.ht_device
        )
        self.assertIsInstance(iris, ht.DNDarray)
        self.assertEqual(iris.shape, self.IRIS.shape)
        self.assertEqual(iris.dtype, ht.int8)
    ...
    def test_load_netcdf_exception(self):
        ...
        # improper argument types
        with self.assertRaises(TypeError):
            ht.load_netcdf(1, "data", device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.load_netcdf("iris.nc", variable=1, device=self.ht_device)
        with self.assertRaises(TypeError):
            ht.load_netcdf("iris.nc", variable="data", split=1.0, device=self.ht_device)

        # file or variable does not exist
        with self.assertRaises(IOError):
            ht.load_netcdf("foo.nc", variable="data", device=self.ht_device)
        with self.assertRaises(IOError):
            ht.load_netcdf("iris.nc", variable="foo", device=self.ht_device)
