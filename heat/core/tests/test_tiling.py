...
class TestSquareDiagTiles(TestCase):
    ...
    if ht.MPI_WORLD.size > 1:
        ...
        def test_local_set_get(self):
            ...
            lcl_slice = m_eq_n_s0.V_local_larray[st_sp[0] : st_sp[1], st_sp[2] : st_sp[3]]
            ...
            # reset base
            m_eq_n_s0.V_local_larray[st_sp[0] : st_sp[1], st_sp[2] : st_sp[3]] = 0
            ...
            sz = st_sp[1] - st_sp[0], st_sp[3] - st_sp[2]
            lcl_slice = m_eq_n_s1.V_local_larray[st_sp[0] : st_sp[1], st_sp[2] : st_sp[3]]
            lcl_shape = m_eq_n_s1_t2.local_get(key=(slice(None), slice(None))).shape
            self.assertEqual(lcl_shape, m_eq_n_s1.lshape)
            self.assertTrue(torch.all(lcl_slice - torch.ones(sz, device=torch_dev) == 0))
            # reset base
            m_eq_n_s1.V_local_larray[st_sp[0] : st_sp[1], st_sp[2] : st_sp[3]] = 0
