...
class TestRounding(TestCase):
    def test_abs(self):
        ...
        # for fabs
        int8_tensor_fabs = ht.arange(-10.5, 10.5, dtype=ht.int8, split=0, device=self.ht_device)
        int8_absolute_values_fabs = ht.fabs(int8_tensor_fabs)
        int16_tensor_fabs = ht.arange(-10.5, 10.5, dtype=ht.int16, split=0, device=self.ht_device)
        int16_absolute_values_fabs = ht.fabs(int16_tensor_fabs)
        int32_tensor_fabs = ht.arange(-10.5, 10.5, dtype=ht.int32, split=0, device=self.ht_device)
        int32_absolute_values_fabs = ht.fabs(int32_tensor_fabs)
        int64_tensor_fabs = ht.arange(-10.5, 10.5, dtype=ht.int64, split=0, device=self.ht_device)
        int64_absolute_values_fabs = ht.fabs(int64_tensor_fabs)
        float32_tensor_fabs = ht.arange(
            -10.5, 10.5, dtype=ht.float32, split=0, device=self.ht_device
        )
        float32_absolute_values_fabs = ht.fabs(float32_tensor_fabs)
        float64_tensor_fabs = ht.arange(
            -10.5, 10.5, dtype=ht.float64, split=0, device=self.ht_device
        )
        ...
        # for abs==absolute
        output_tensor = ht.zeros(20, split=0, device=self.ht_device)
        ...
        # for fabs
        output_tensor_fabs = ht.zeros(21, split=0, device=self.ht_device)
        ...
        # dtype parameter
        # for abs==absolute
        int64_tensor = ht.arange(-10, 10, dtype=ht.int64, device=self.ht_device)
        ...
        # for fabs
        float32_unsplit_tensor_fabs = ht.arange(
            -10.5, 10.5, dtype=ht.float32, device=self.ht_device
        )
    ...
    def test_ceil(self):
        ...
        comparison = torch.arange(
            start, end, step, dtype=torch.float64, device=self.torch_device
        ).ceil()

        # exponential of float32
        float32_tensor = ht.arange(start, end, step, dtype=ht.float32, device=self.ht_device)
        ...
        float64_tensor = ht.arange(start, end, step, dtype=ht.float64, device=self.ht_device)
    ...
    def test_clip(self):
        ...
        # float tensor
        float32_tensor = ht.arange(elements, dtype=ht.float32, split=0, device=self.ht_device)
        ...
        # long tensor
        int64_tensor = ht.arange(elements, dtype=ht.int64, split=0, device=self.ht_device)
        ...
        # test the exceptions
        with self.assertRaises(TypeError):
            ht.clip(torch.arange(10, device=self.torch_device), 2, 5)
        with self.assertRaises(ValueError):
            ht.arange(20, device=self.ht_device).clip(None, None)
        with self.assertRaises(TypeError):
            ht.clip(ht.arange(20), 5, 15, out=torch.arange(20, device=self.torch_device))
    ...
    def test_trunc(self):
        ...
        comparison = torch.tensor(base_array, dtype=torch.float64, device=self.torch_device).trunc()

        # trunc of float32
        float32_tensor = ht.array(base_array, dtype=ht.float32, device=self.ht_device)
        ...
        # trunc of float64
        float64_tensor = ht.array(base_array, dtype=ht.float64, device=self.ht_device)
