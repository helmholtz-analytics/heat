...
class TestArithmetics(unittest.TestCase):
    ...
    def test_add(self):
        result = ht.array([[3.0, 4.0], [5.0, 6.0]], device=heat_device)
    ...
    def test_bitwise_and(self):
        an_int_tensor = ht.array([[1, 2], [3, 4]], device=heat_device)
        an_int_vector = ht.array([2, 2], device=heat_device)
        another_int_vector = ht.array([2, 2, 2, 2], device=heat_device)
        int_result = ht.array([[0, 2], [2, 0]], device=heat_device)

        a_boolean_vector = ht.array([False, True, False, True], device=heat_device)
        another_boolean_vector = ht.array([False, False, True, True], device=heat_device)
        boolean_result = ht.array([False, False, False, True], device=heat_device)
    ...
    def test_bitwise_or(self):
        ...
        an_int_vector = ht.array([2, 2], device=heat_device)
        another_int_vector = ht.array([2, 2, 2, 2], device=heat_device)
        int_result = ht.array([[3, 2], [3, 6]], device=heat_device)

        a_boolean_vector = ht.array([False, True, False, True], device=heat_device)
        another_boolean_vector = ht.array([False, False, True, True], device=heat_device)
        boolean_result = ht.array([False, True, True, True], device=heat_device)
    ...
    def test_bitwise_xor(self):
        ...
        an_int_vector = ht.array([2, 2], device=heat_device)
        another_int_vector = ht.array([2, 2, 2, 2], device=heat_device)
        int_result = ht.array([[3, 0], [1, 6]], device=heat_device)

        a_boolean_vector = ht.array([False, True, False, True], device=heat_device)
        another_boolean_vector = ht.array([False, False, True, True], device=heat_device)
        boolean_result = ht.array([False, True, True, False], device=heat_device)
    ...
    def test_diff(self):
        ht_array = ht.random.rand(20, 20, 20, split=None, device=heat_device)
        arb_slice = [0] * 3
            ...
            for ax in range(dim + 1):  # loop over the possible axis values
                for sp in range(dim + 1):  # loop over the possible split values
                    ...
                    for nl in range(1, 4):
                        ...
                        np_diff = ht.array(np.diff(np_array, n=nl, axis=ax), device=heat_device)
        ...
        np_diff = ht.array(np.diff(np_array, n=2), device=heat_device)
        self.assertTrue(ht.equal(ht_diff, np_diff))
        self.assertEqual(ht_diff.split, None)
        self.assertEqual(ht_diff.dtype, ht_array.dtype)

        ht_array = ht.random.rand(20, 20, 20, split=1, dtype=ht.float64, device=heat_device)
    ...
    def test_div(self):
        result = ht.array([[0.5, 1.0], [1.5, 2.0]], device=heat_device)
        commutated_result = ht.array([[2.0, 1.0], [2.0 / 3.0, 0.5]], device=heat_device)
    ...
    def test_fmod(self):
        result = ht.array([[1.0, 0.0], [1.0, 0.0]], device=heat_device)
        an_int_tensor = ht.array([[5, 3], [4, 1]], device=heat_device)
        integer_result = ht.array([[1, 1], [0, 1]], device=heat_device)
        commutated_result = ht.array([[0.0, 0.0], [2.0, 2.0]], device=heat_device)
        zero_tensor = ht.zeros((2, 2), device=heat_device)

        a_float = ht.array([5.3], device=heat_device)
        another_float = ht.array([1.9], device=heat_device)
        result_float = ht.array([1.5], device=heat_device)
    ...
    def test_mod(self):
        a_tensor = ht.array([[1, 4], [2, 2]], device=heat_device)
        another_tensor = ht.array([[1, 2], [3, 4]], device=heat_device)
        a_result = ht.array([[0, 0], [2, 2]], device=heat_device)
        another_result = ht.array([[1, 0], [0, 0]], device=heat_device)
    ...
    def test_mul(self):
        result = ht.array([[2.0, 4.0], [6.0, 8.0]], device=heat_device)
    ...
    def test_pow(self):
        result = ht.array([[1.0, 4.0], [9.0, 16.0]], device=heat_device)
        commutated_result = ht.array([[2.0, 4.0], [8.0, 16.0]], device=heat_device)
    ...
    def test_prod(self):
        ...
        shape_noaxis = ht.ones(array_len, device=heat_device)
        ...
        out_noaxis = ht.zeros((1,), device=heat_device)
        ...
        shape_noaxis_split = ht.arange(1, array_len, split=0, device=heat_device)
        ...
        shape_noaxis = ht.full((3, 3, 3), 2, device=heat_device)
        no_axis_prod = shape_noaxis.prod()
        ...
        shape_noaxis_split_axis = ht.full((3, 3, 3), 2, split=0, device=heat_device)
        ...
        out_noaxis = ht.zeros((1, 2, 3, 5), device=heat_device)
        ...
        shape_split_axis_tuple = ht.ones((3, 4, 5), split=1, device=heat_device)
        shape_split_axis_tuple_prod = shape_split_axis_tuple.prod(axis=(-2, -3))
        expected_result = ht.ones((5,), device=heat_device)
        ...
        # exceptions
        with self.assertRaises(ValueError):
            ht.ones(array_len, device=heat_device).prod(axis=1)
        with self.assertRaises(ValueError):
            ht.ones(array_len, device=heat_device).prod(axis=-2)
        with self.assertRaises(ValueError):
            ht.ones((4, 4), device=heat_device).prod(axis=0, out=out_noaxis)
        with self.assertRaises(TypeError):
            ht.ones(array_len, device=heat_device).prod(axis="bad_axis_type")
    ...
    def test_sub(self):
        result = ht.array([[-1.0, 0.0], [1.0, 2.0]], device=heat_device)
        minus_result = ht.array([[1.0, 0.0], [-1.0, -2.0]], device=heat_device)
    ...
    def test_sum(self):
        ...
        # check sum over all float elements of split 1d tensor
        shape_noaxis_split = ht.arange(array_len, split=0, device=heat_device)
        ...
        # check sum over all float elements of 3d tensor locally
        shape_noaxis = ht.ones((3, 3, 3), device=heat_device)
        no_axis_sum = shape_noaxis.sum()
        ...
        # check sum over all float elements of split 3d tensor
        shape_noaxis_split_axis = ht.ones((3, 3, 3), split=0, device=heat_device)
        ...
        expected_result = ht.ones((5,), device=heat_device) * 12.0
        ...
        # exceptions
        with self.assertRaises(ValueError):
            ht.ones(array_len, device=heat_device).sum(axis=1)
        with self.assertRaises(ValueError):
            ht.ones(array_len, device=heat_device).sum(axis=-2)
        with self.assertRaises(ValueError):
            ht.ones((4, 4), device=heat_device).sum(axis=0, out=out_noaxis)
        with self.assertRaises(TypeError):
            ht.ones(array_len, device=heat_device).sum(axis="bad_axis_type")
    ...
    def test_right_hand_side_operations(self):
        ...
        --------
        >>> import heat as ht
        >>> T = ht.float32([[1., 2.], [3., 4.]], device=heat_device)
        >>> assert T * 3 == 3 * T
        """
        operators = (
        ...
        tensor = ht.float32([[1, 4], [2, 3]], device=heat_device)
        num = 3
