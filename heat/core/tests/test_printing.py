...
class TestPrintingGPU(TestCase):
    def test_print_GPU(self):
        ...
        a0 = ht.arange(2**21, dtype=ht.float32).reshape((2**6, 2**7, 2**8)).resplit_(0)
        a1 = ht.arange(2**21, dtype=ht.float32).reshape((2**6, 2**7, 2**8)).resplit_(1)
        a2 = ht.arange(2**21, dtype=ht.float32).reshape((2**6, 2**7, 2**8)).resplit_(2)
        print(a0, a1, a2)
