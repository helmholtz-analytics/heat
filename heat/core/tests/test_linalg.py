...
class TestLinalg(unittest.TestCase):
    ...
    def test_matmul(self):
        ...
        n, m = 21, 31
        j, k = m, 45
        a_torch = torch.ones((n, m), device=torch_device)
        a_torch[0] = torch.arange(1, m + 1, device=torch_device)
        a_torch[:, -1] = torch.arange(1, n + 1, device=torch_device)
        b_torch = torch.ones((j, k), device=torch_device)
        ...
        if a.comm.size > 1:
            ...
            # vector matrix mult:
            # a -> vector
            a_torch = torch.ones((m), device=torch_device)
            b_torch = torch.ones((j, k), device=torch_device)
            b_torch[0] = torch.arange(1, k + 1, device=torch_device)
            b_torch[:, 0] = torch.arange(1, j + 1, device=torch_device)
            # splits None None
    ...
    if extended_tests:
        st_whole = torch.randn(70, 70, device=torch_device)
    ...
    def test_tril(self):
        ...
        comparison = torch.ones((5, 5), device=torch_device).tril()
        ...
        comparison = torch.ones((5, 5), device=torch_device).tril(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (5, 5))
        self.assertEqual(result.lshape, (5, 5))
        self.assertEqual(result.split, None)
        ...
        comparison = torch.ones((4, 5), device=torch_device).tril()
        self.assertIsInstance(result, ht.DNDarray)
        ...
        comparison = torch.ones((4, 5), device=torch_device).tril(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (4, 5))
        self.assertEqual(result.lshape, (4, 5))
        self.assertEqual(result.split, None)
        ...
        result = local_ones.tril()
        comparison = torch.ones((5, 6), device=torch_device).tril()
        self.assertIsInstance(result, ht.DNDarray)
        ...
        result = local_ones.tril(k=2)
        comparison = torch.ones((5, 6), device=torch_device).tril(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (3, 4, 5, 6))
        self.assertEqual(result.lshape, (3, 4, 5, 6))
        self.assertEqual(result.split, None)
    ...
    def test_triu(self):
        ...
        comparison = torch.ones((5, 5), device=torch_device).triu()
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (5, 5))
        self.assertEqual(result.lshape, (5, 5))
        self.assertEqual(result.split, None)
        ...
        comparison = torch.ones((5, 5), device=torch_device).triu(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (5, 5))
        self.assertEqual(result.lshape, (5, 5))
        self.assertEqual(result.split, None)
        ...
        comparison = torch.ones((4, 5), device=torch_device).triu()
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (4, 5))
        self.assertEqual(result.lshape, (4, 5))
        self.assertEqual(result.split, None)
        ...
        comparison = torch.ones((4, 5), device=torch_device).triu(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (4, 5))
        self.assertEqual(result.lshape, (4, 5))
        self.assertEqual(result.split, None)
        ...
        # 2D+ case, no offset, data is not split, module-level call
        result = local_ones.triu()
        comparison = torch.ones((5, 6), device=torch_device).triu()
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (3, 4, 5, 6))
        self.assertEqual(result.lshape, (3, 4, 5, 6))
        self.assertEqual(result.split, None)
        ...
        # 2D+ case, positive offset, data is not split, module-level call
        result = local_ones.triu(k=2)
        comparison = torch.ones((5, 6), device=torch_device).triu(diagonal=2)
        self.assertIsInstance(result, ht.DNDarray)
        self.assertEqual(result.shape, (3, 4, 5, 6))
        self.assertEqual(result.lshape, (3, 4, 5, 6))
        self.assertEqual(result.split, None)
