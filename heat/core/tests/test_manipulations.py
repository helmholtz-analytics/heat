...
class TestManipulations(BasicTest):
    ...
    def test_diagonal(self):
        ...
        self.assertTrue(
            torch.equal(
                res._DNDarray__array,
                torch.arange(size, device=torch_device).repeat(size + 1).reshape(size + 1, size),
            )
        )
    ...
    def test_sort(self):
        ...
        exp_axis_one, exp_indices = (
            torch.arange(size, device=torch_device).reshape(1, size).sort(dim=1, descending=True)
        )
        ...
        if torch.cuda.is_available() and data.device == ht.gpu and size < 4:
            indices_axis_zero = torch.tensor(
                [[0, 2, 2], [3, 2, 0]], dtype=torch.int32, device=torch_device
            )
        else:
            indices_axis_zero = torch.tensor(
                [[0, 2, 2], [3, 0, 0]], dtype=torch.int32, device=torch_device
            )
