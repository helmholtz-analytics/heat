...
class TestManipulations(BasicTest):
    ...
    def test_concatenate(self):
        ...
        # s0    s1  axis
        # None None 0
        x = ht.zeros((16, 15), split=None, device=self.ht_device)
        y = ht.ones((16, 15), split=None, device=self.ht_device)
        ...
        # None 0 0
        x = ht.zeros((16, 15), split=None, device=self.ht_device)
        y = ht.ones((16, 15), split=0, device=self.ht_device)
        res = ht.concatenate((x, y), axis=0)
        ...
        # None 1 1
        x = ht.zeros((16, 15), split=None, device=self.ht_device)
        y = ht.ones((16, 15), split=1, device=self.ht_device)
        res = ht.concatenate((x, y), axis=1)
        self.assertEqual(res.gshape, (16, 30))
        self.assertEqual(res.dtype, ht.float)
        _, _, chk = res.comm.chunk((16, 30), res.split)
        ...
        # # =============================================
        # # 0 None 0
        x = ht.zeros((16, 15), split=0, device=self.ht_device)
        y = ht.ones((16, 15), split=None, device=self.ht_device)
        res = ht.concatenate((x, y), axis=0)
        self.assertEqual(res.gshape, (32, 15))
        self.assertEqual(res.dtype, ht.float)
        ...
        # =============================================
        # 1 None 0
        x = ht.zeros((16, 15), split=1, device=self.ht_device)
        y = ht.ones((16, 15), split=None, device=self.ht_device)
        res = ht.concatenate((x, y), axis=0)
        self.assertEqual(res.gshape, (32, 15))
        self.assertEqual(res.dtype, ht.float)
        ...
        # =============================================
        x = ht.zeros((16, 15, 14), split=2, device=self.ht_device)
        y = ht.ones((16, 15, 14), split=2, device=self.ht_device)
        # 2 2 0
        res = ht.concatenate((x, y), axis=0)
        ...
        #
        # =============================================
        y = ht.ones((16, 15, 14), split=None, device=self.ht_device)
        # 2 None 1
        res = ht.concatenate((x, y), axis=1)
        self.assertEqual(res.gshape, (16, 30, 14))
        self.assertEqual(res.dtype, ht.float)
        ...
        # =============================================
        x = ht.zeros((16, 15, 14), split=None, device=self.ht_device)
        y = ht.ones((16, 15, 14), split=2, device=self.ht_device)
        # None 2 0
        res = ht.concatenate((x, y), axis=0)
        self.assertEqual(res.gshape, (32, 15, 14))
        ...
        # vectors
        # None None 0
        x = ht.zeros((16,), split=None, device=self.ht_device)
        y = ht.ones((16,), split=None, device=self.ht_device)
        res = ht.concatenate((x, y), axis=0)
        self.assertEqual(res.gshape, (32,))
        self.assertEqual(res.dtype, ht.float)
        # None 0 0
        y = ht.ones((16,), split=0, device=self.ht_device)
        res = ht.concatenate((x, y), axis=0)
        self.assertEqual(res.gshape, (32,))
        self.assertEqual(res.dtype, ht.float)
        ...
        # 0 0 0
        x = ht.ones((16,), split=0, dtype=ht.float64, device=self.ht_device)
        res = ht.concatenate((x, y), axis=0)
        self.assertEqual(res.gshape, (32,))
        ...
        self.assertEqual(res.lshape, tuple(lshape))
        # 0 None 0
        x = ht.ones((16,), split=0, device=self.ht_device)
        y = ht.ones((16,), split=None, dtype=ht.int64, device=self.ht_device)
        res = ht.concatenate((x, y), axis=0)
        self.assertEqual(res.gshape, (32,))
        self.assertEqual(res.dtype, ht.float64)
        _, _, chk = res.comm.chunk((32,), res.split)
        ...
        with self.assertRaises(ValueError):
            ht.concatenate(
                (
                    ht.zeros((6, 3, 5), device=self.ht_device),
                    ht.zeros((4, 5, 1), device=self.ht_device),
                )
            )
        ...
        with self.assertRaises(ValueError):
            ht.concatenate((x, ht.zeros((2, 2), device=self.ht_device)), axis=0)
        ...
        with self.assertRaises(ValueError):
            ht.concatenate(
                (
                    ht.zeros((12, 12), device=self.ht_device),
                    ht.zeros((2, 2), device=self.ht_device),
                ),
                axis=0,
            )
        with self.assertRaises(RuntimeError):
            ht.concatenate(
                (
                    ht.zeros((2, 2), split=0, device=self.ht_device),
                    ht.zeros((2, 2), split=1, device=self.ht_device),
                ),
                axis=0,
            )
    ...
    def test_diag(self):
        ...
        data = torch.arange(size * 2, device=self.torch_device)
        a = ht.array(data, device=self.ht_device)
        res = ht.diag(a)
        ...
        a = ht.array(data, split=0, device=self.ht_device)
        res = ht.diag(a)
        ...
        a = ht.random.rand(15, 20, 5, split=1, device=self.ht_device)
        res_1 = ht.diag(a)
        res_2 = ht.diagonal(a)
        ...
        a = ht.arange(size, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.diag(a, offset="3")

        a = ht.empty([], device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.diag(a)

        if rank == 0:
            data = torch.ones(size, dtype=torch.int32, device=self.torch_device)
        else:
            data = torch.empty(0, dtype=torch.int32, device=self.torch_device)
        a = ht.array(data, is_split=0, device=self.ht_device)
        res = ht.diag(a)
        self.assertTrue(
            torch.equal(
                res[rank, rank]._DNDarray__array,
                torch.tensor(1, dtype=torch.int32, device=self.torch_device),
            )
        )
    ...
    def test_diagonal(self):
        ...
        data = torch.arange(size, device=self.torch_device).repeat(size).reshape(size, size)
        a = ht.array(data, device=self.ht_device)
        res = ht.diagonal(a)
        self.assertTrue(
            torch.equal(res._DNDarray__array, torch.arange(size, device=self.torch_device))
        )
        ...
        res = ht.diagonal(a)
        self.assertTrue(
            torch.equal(res._DNDarray__array, torch.tensor([rank], device=self.torch_device))
        )
        self.assertEqual(res.split, 0)

        a = ht.array(data, split=1, device=self.ht_device)
        ...
        data = (
            torch.arange(size + 1, device=self.torch_device)
            .repeat(size + 1)
        ...
        self.assertTrue(
            torch.equal(res._DNDarray__array, torch.arange(size + 1, device=self.torch_device))
        )
        res = ht.diagonal(a, offset=1)
        self.assertTrue(
            torch.equal(res._DNDarray__array, torch.arange(1, size + 1, device=self.torch_device))
        )
        res = ht.diagonal(a, offset=-1)
        self.assertTrue(
            torch.equal(res._DNDarray__array, torch.arange(0, size, device=self.torch_device))
        )
        ...
        res.balance_()
        self.assertTrue(
            torch.equal(res._DNDarray__array, torch.tensor([rank + 1], device=self.torch_device))
        )
        res = ht.diagonal(a, offset=-1)
        res.balance_()
        self.assertTrue(
        ...
        data = (
            torch.arange(size * 2 + 10, device=self.torch_device)
            .repeat(size * 2 + 10)
        ...
        self.assertTrue(
            torch.equal(
                res._DNDarray__array, torch.arange(10, 10 + size * 2, device=self.torch_device)
            )
        )
        res = ht.diagonal(a, offset=-10)
        self.assertTrue(
            torch.equal(res._DNDarray__array, torch.arange(0, size * 2, device=self.torch_device))
        )
        ...
        self.assertTrue(
            torch.equal(
                ...
                torch.tensor([10 + rank * 2, 11 + rank * 2], device=self.torch_device),
            )
        )
        res = ht.diagonal(a, offset=-10)
        res.balance_()
        self.assertTrue(
            torch.equal(
                res._DNDarray__array,
                torch.tensor([rank * 2, 1 + rank * 2], device=self.torch_device),
            )
        )
        ...
        self.assertTrue(
            torch.equal(
                res._DNDarray__array,
                torch.arange(1, size + 1, device=self.torch_device)
                .repeat(size + 1)
                .reshape(size + 1, size),
            )
        )
        ...
        self.assertTrue(
            torch.equal(
                res._DNDarray__array,
                torch.arange(size, device=self.torch_device)
                .repeat(size + 1)
                .reshape(size + 1, size),
            )
        )
        ...
        self.assertTrue(
            torch.equal(
                res._DNDarray__array,
                torch.arange(size + 1, device=self.torch_device).reshape(size + 1, 1),
            )
        )
        ...
        self.assertTrue(
            torch.equal(
                res._DNDarray__array,
                torch.empty((size + 1, 0), dtype=torch.int64, device=self.torch_device),
            )
        )
    ...
    def test_expand_dims(self):
        # vector data
        a = ht.arange(10, device=self.ht_device)
        ...
        a = ht.arange(12, device=self.ht_device)
        b = a.expand_dims(1)
        ...
        a = ht.empty((3, 4, 5), device=self.ht_device)
        b = a.expand_dims(1)
        ...
        a = ht.empty((3, 4, 5), split=1, device=self.ht_device)
        b = a.expand_dims(-2)
        ...
        a = ht.empty((3, 4, 5), split=2, device=self.ht_device)
        b = a.expand_dims(-3)
        ...
        with self.assertRaises(TypeError):
            ht.empty((3, 4, 5), device=self.ht_device).expand_dims("1")
        with self.assertRaises(ValueError):
            ht.empty((3, 4, 5), device=self.ht_device).expand_dims(4)
        with self.assertRaises(ValueError):
    ...
    def test_flip(self):
        a = ht.array([1, 2], device=self.ht_device)
        r_a = ht.array([2, 1], device=self.ht_device)
        self.assertTrue(ht.equal(ht.flip(a, 0), r_a))

        a = ht.array([[1, 2], [3, 4]], device=self.ht_device)
        r_a = ht.array([[4, 3], [2, 1]], device=self.ht_device)
        self.assertTrue(ht.equal(ht.flip(a), r_a))

        a = ht.array(
            [[2, 3], [4, 5], [6, 7], [8, 9]], split=1, dtype=ht.float32, device=self.ht_device
        )
        r_a = ht.array(
            [[9, 8], [7, 6], [5, 4], [3, 2]], split=1, dtype=ht.float32, device=self.ht_device
        )
        self.assertTrue(ht.equal(ht.flip(a, [0, 1]), r_a))

        a = ht.array(
            [[[0, 1], [2, 3]], [[4, 5], [6, 7]]], split=0, dtype=ht.uint8, device=self.ht_device
        )
        r_a = ht.array(
            [[[3, 2], [1, 0]], [[7, 6], [5, 4]]], split=0, dtype=ht.uint8, device=self.ht_device
        )
    ...
    def test_flipud(self):
        ...
        b = ht.array([[1, 2], [3, 4]], device=self.ht_device)
        r_b = ht.array([[3, 4], [1, 2]], device=self.ht_device)
    ...
    def test_hstack(self):
        ...
        # MM===================================
        # NN,
        a = ht.ones((10, 12), split=None, device=self.ht_device)
        b = ht.ones((10, 12), split=None, device=self.ht_device)
        res = ht.hstack((a, b))
        self.assertEqual(res.shape, (10, 24))
        # 11,
        a = ht.ones((10, 12), split=1, device=self.ht_device)
        b = ht.ones((10, 12), split=1, device=self.ht_device)
        res = ht.hstack((a, b))
        self.assertEqual(res.shape, (10, 24))

        # VM===================================
        # NN,
        a = ht.ones((12,), split=None, device=self.ht_device)
        b = ht.ones((12, 10), split=None, device=self.ht_device)
        res = ht.hstack((a, b))
        self.assertEqual(res.shape, (12, 11))
        # 00
        a = ht.ones((12,), split=0, device=self.ht_device)
        b = ht.ones((12, 10), split=0, device=self.ht_device)
        res = ht.hstack((a, b))
        self.assertEqual(res.shape, (12, 11))

        # MV===================================
        # NN,
        a = ht.ones((12, 10), split=None, device=self.ht_device)
        b = ht.ones((12,), split=None, device=self.ht_device)
        res = ht.hstack((a, b))
        self.assertEqual(res.shape, (12, 11))
        # 00
        a = ht.ones((12, 10), split=0, device=self.ht_device)
        b = ht.ones((12,), split=0, device=self.ht_device)
        res = ht.hstack((a, b))
        self.assertEqual(res.shape, (12, 11))
    ...
    def test_sort(self):
        size = ht.MPI_WORLD.size
        rank = ht.MPI_WORLD.rank
        tensor = torch.arange(size, device=self.torch_device).repeat(size).reshape(size, size)

        data = ht.array(tensor, split=None, device=self.ht_device)
        ...
        data = ht.array(tensor, split=0, device=self.ht_device)

        exp_axis_zero = torch.arange(size, device=self.torch_device).reshape(1, size)
        exp_indices = torch.tensor([[rank] * size], device=self.torch_device)
        ...
        data = ht.array(tensor, split=1, device=self.ht_device)

        exp_axis_zero = torch.tensor(rank, device=self.torch_device).repeat(size).reshape(size, 1)
        indices_axis_zero = torch.arange(size, dtype=torch.int64, device=self.torch_device).reshape(
            size, 1
        )
        result, result_indices = ht.sort(data, axis=0, descending=True)
        self.assertTrue(torch.equal(result._DNDarray__array, exp_axis_zero))
        ...
        exp_axis_one = (
            torch.tensor(size - rank - 1, device=self.torch_device).repeat(size).reshape(size, 1)
        )
        result, result_indices = ht.sort(data, descending=True, axis=1)
        self.assertTrue(torch.equal(result._DNDarray__array, exp_axis_one))
        ...
        exp_axis_zero = torch.tensor(
            [[2, 3, 0], [0, 2, 3]], dtype=torch.int32, device=self.torch_device
        )
        if torch.cuda.is_available() and data.device == ht.gpu and size < 4:
            indices_axis_zero = torch.tensor(
                [[0, 2, 2], [3, 2, 0]], dtype=torch.int32, device=self.torch_device
            )
        else:
            indices_axis_zero = torch.tensor(
                [[0, 2, 2], [3, 0, 0]], dtype=torch.int32, device=self.torch_device
            )
        ...
        data = ht.array(tensor, split=1, device=self.ht_device)
        exp_axis_one = torch.tensor([[2, 2, 3]], dtype=torch.int32, device=self.torch_device)
        indices_axis_one = torch.tensor([[0, 1, 1]], dtype=torch.int32, device=self.torch_device)
        ...
        data = ht.array(tensor, split=2, device=self.ht_device)
        exp_axis_two = torch.tensor([[2], [2]], dtype=torch.int32, device=self.torch_device)
        indices_axis_two = torch.tensor([[0], [1]], dtype=torch.int32, device=self.torch_device)
        ...
        #
        out = ht.empty_like(data, device=self.ht_device)
        ...
        rank = ht.MPI_WORLD.rank
        data = ht.random.randn(100, 1, split=0, device=self.ht_device)
    ...
    def test_squeeze(self):
        torch.manual_seed(1)
        data = ht.random.randn(1, 4, 5, 1, device=self.ht_device)
        ...
        data = ht.triu(ht.ones((1, size, size), split=1, device=self.ht_device), k=1)
    ...
    def test_unique(self):
        size = ht.MPI_WORLD.size
        rank = ht.MPI_WORLD.rank
        torch_array = torch.arange(size, dtype=torch.int32, device=self.torch_device).expand(
            size, size
        )
        split_zero = ht.array(torch_array, split=0, device=self.ht_device)

        exp_axis_none = ht.array([rank], dtype=ht.int32, device=self.ht_device)
        res = split_zero.unique(sorted=True)
        self.assertTrue((res._DNDarray__array == exp_axis_none._DNDarray__array).all())

        exp_axis_zero = ht.arange(size, dtype=ht.int32, device=self.ht_device).expand_dims(0)
        res = ht.unique(split_zero, sorted=True, axis=0)
        self.assertTrue((res._DNDarray__array == exp_axis_zero._DNDarray__array).all())

        exp_axis_one = ht.array([rank], dtype=ht.int32, device=self.ht_device).expand_dims(0)
        split_zero_transposed = ht.array(
            torch_array.transpose(0, 1), split=0, device=self.ht_device
        )
        ...
        split_one = ht.array(torch_array, dtype=ht.int32, split=1, device=self.ht_device)

        exp_axis_none = ht.arange(size, dtype=ht.int32, device=self.ht_device)
        res = ht.unique(split_one, sorted=True)
        self.assertTrue((res._DNDarray__array == exp_axis_none._DNDarray__array).all())

        exp_axis_zero = ht.array([rank], dtype=ht.int32, device=self.ht_device).expand_dims(0)
        res = ht.unique(split_one, sorted=False, axis=0)
        self.assertTrue((res._DNDarray__array == exp_axis_zero._DNDarray__array).all())

        exp_axis_one = ht.array([rank] * size, dtype=ht.int32, device=self.ht_device).expand_dims(1)
        res = ht.unique(split_one, sorted=True, axis=1)
        self.assertTrue((res._DNDarray__array == exp_axis_one._DNDarray__array).all())

        torch_array = torch.tensor(
            [[1, 2], [2, 3], [1, 2], [2, 3], [1, 2]], dtype=torch.int32, device=self.torch_device
        )
        data = ht.array(torch_array, split=0, device=self.ht_device)
        ...
        data_split_none = ht.array(torch_array, device=self.ht_device)
    ...
    def test_vstack(self):
        ...
        # VM===================================
        # NN,
        a = ht.ones((10,), split=None, device=self.ht_device)
        b = ht.ones((12, 10), split=None, device=self.ht_device)
        res = ht.vstack((a, b))
        self.assertEqual(res.shape, (13, 10))
        # 00
        a = ht.ones((10,), split=0, device=self.ht_device)
        b = ht.ones((12, 10), split=0, device=self.ht_device)
        res = ht.vstack((a, b))
        self.assertEqual(res.shape, (13, 10))
        ...
        # NN,
        a = ht.ones((12, 10), split=None, device=self.ht_device)
        b = ht.ones((10,), split=None, device=self.ht_device)
        res = ht.vstack((a, b))
        self.assertEqual(res.shape, (13, 10))
        # 00
        a = ht.ones((12, 10), split=0, device=self.ht_device)
        b = ht.ones((10,), split=0, device=self.ht_device)
        res = ht.vstack((a, b))
        self.assertEqual(res.shape, (13, 10))
