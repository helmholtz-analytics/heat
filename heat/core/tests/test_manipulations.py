...
class TestManipulations(unittest.TestCase):
    ...
    def test_unique_2(self):
        ...
        exp_axis_one = ht.array([rank], dtype=ht.int32).expand_dims(0)
        split_zero_transposed = ht.array(torch_array.transpose(0, 1), split=0)
        print('rank', rank, 'local', split_zero_transposed)
        res = ht.unique(split_zero_transposed, sorted=True, axis=1)
        print('res', res)
        self.assertTrue((res._DNDarray__array == exp_axis_one._DNDarray__array).all())

    def test_unique_exp(self):
        print('Rank', ht.MPI_WORLD.rank)
        tensor = torch.tensor([[[-0.95354314, 1.14044643, 0.07100701, -2.29106916, 0.7077135, -0.74500277],
                                [-1.23667462, 0.36949895, -1.03097934, 0.01426911, -0.70604335, -0.87842448]],
                               [[0.24400927, -1.09485638, -0.89672624, -0.86396395, -0.37699114, -1.13366005],
                                [0.02036728, 1.14189502, -0.35003392, -0.85320526, 1.12145987, -0.64987599]],
                               [[0.49950128, -0.29610317, -0.27155303, 1.11548237, 2.08973799, -0.60534049],
                                [-0.18361788, -0.24160703, 0.83607112, 1.77136763, 0.50675678, 0.56090804]],
                               [[2.37447139, 1.98826522, 1.76301019, -1.3418815, -1.83614516, 1.61957017],
                                [1.07203462, -0.03574395, -0.26640549, -1.18455919, 0.80088419, -1.24503972]],
                               [[0.00360497, 0.95737742, 0.63510542, 1.34776782, 0.93401487, 0.49026929],
                                [1.15711687, -0.09189166, -1.06685467, 0.60830886, 0.43360934, 1.19698626]]])
        # tensor = torch.tensor([[2, 1], [1, 2], [1, 3]])
        data = ht.array(tensor, split=1)
        print('local', data)
        res = ht.unique(data, sorted=True, axis=0)
        print('res', res)
        res = res.resplit(None)
        print('resplit', res)
        np_res = np.unique(tensor.numpy(), axis=0)
        print('np_res', np_res)
        self.assertTrue(np.array_equal(res._DNDarray__array.numpy(), np_res))
        self.fail()
