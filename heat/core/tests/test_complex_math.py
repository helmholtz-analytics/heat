...
class TestComplex(TestCase):
    ...
    def test_angle(self):
        ...
        res = ht.array([  0.,  90.,  45., 135., -45.], dtype=ht.float32, device=self.device, split=0)
    ...
    def test_conjugate(self):
        a = ht.array([1.0, 1.0j, 1+1j, -2+2j, 3 - 3j], split=0)
        conj = ht.conjugate(a)
        res = ht.array([ 1-0j, -1j, 1-1j, -2-2j, 3+3j], dtype=ht.complex64, device=self.device, split=0)
    ...
    def test_imag(self):
        a = ht.array([1.0, 1.0j, 1+1j, -2+2j, 3 - 3j], split=0)
        imag = ht.imag(a)
        res = ht.array([ 0.,  1.,  1.,  2., -3.], dtype=ht.float32, device=self.device, split=0)
    ...
    def test_real(self):
        a = ht.array([1.0, 1.0j, 1+1j, -2+2j, 3 - 3j], split=0)
        real = ht.real(a)
        res = ht.array([ 1.,  0.,  1., -2.,  3.], dtype=ht.float32, device=self.device, split=0)
