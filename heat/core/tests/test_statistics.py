...
class TestStatistics(unittest.TestCase):
    ...
    def test_average(self):
        ...
        random_weights = ht.array(
            torch.randn((3 * size,), dtype=torch.float64, device=torch_device), device=heat_device
        )
        ...
        random_weights = ht.random.randn(random_5d.gshape[axis] + 1, device=heat_device)
        with self.assertRaises(ValueError):
            ht.average(random_5d, weights=random_weights, axis=axis)
        random_weights = ht.zeros((random_5d.gshape[axis]), device=heat_device)
