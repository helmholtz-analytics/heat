...
class TestStatistics(unittest.TestCase):
    def test_argmax(self):
        torch.manual_seed(1)
        data = ht.random.randn(3, 4, 5, device=heat_device)
        ...
        data = ht.arange(-10, 10, split=0, device=heat_device)
        ...
        self.assertEqual(result.split, None)
        self.assertTrue((result._DNDarray__array == torch.tensor([19], device=torch_device)))

        # 2D split tensor, along the axis
        data = ht.array(ht.random.randn(4, 5, device=heat_device), is_split=0, device=heat_device)
        ...
        data = ht.tril(ht.ones((size, size), split=0, device=heat_device), k=-1)
        ...
        output = ht.empty((size,), device=heat_device)
    ...
    def test_argmin(self):
        ...
        # 2D split tensor, along the axis
        data = ht.array(ht.random.randn(4, 5), is_split=0, device=heat_device)
        ...
        # 2D split tensor, across the axis
        size = ht.MPI_WORLD.size * 2
        data = ht.triu(ht.ones((size, size), split=0, device=heat_device), k=1)
    ...
    def test_cov(self):
        x = ht.array([[0, 2], [1, 1], [2, 0]], dtype=ht.float, split=1, device=heat_device).T
        if x.comm.size < 3:
            cov = ht.cov(x)
            actual = ht.array([[1, -1], [-1, 1]], split=0, device=heat_device)
        ...
        self.assertTrue(
            ht.allclose(ht.array(np_cov, dtype=ht.float, device=heat_device) - ht_cov, 0, atol=1e-4)
        )
        ...
        if 1 < x.comm.size < 5:
            htdata = ht.load("heat/datasets/data/iris.csv", sep=";", split=1, device=heat_device)
            np_cov = np.cov(data, rowvar=False)
            ht_cov = ht.cov(htdata, rowvar=False)
            self.assertTrue(
                ht.allclose(ht.array(np_cov, dtype=ht.float, device=heat_device), ht_cov, atol=1e-4)
            )
        ...
        with self.assertRaises(ValueError):
            ht.cov(ht.zeros((1, 2, 3), device=heat_device))
        with self.assertRaises(ValueError):
            ht.cov(htdata, ht.zeros((1, 2, 3), device=heat_device))
        with self.assertRaises(ValueError):
    ...
    def test_average(self):
        ...
        ht_array = ht.array(data, dtype=float, device=heat_device)
        ...
        random_weights = ht.array(
            torch.randn((3 * size,), dtype=torch.float64, device=torch_device),
            split=0,
            device=heat_device,
        )
        ...
        random_volume = ht.random.randn(3, 3, 3, split=0, device=heat_device)
        ...
        random_weights = ht.random.randn(random_5d.gshape[axis], split=0, device=heat_device)
        ...
        random_weights = ht.random.randn(
            random_5d.gshape[axis], random_5d.gshape[axis + 1], device=heat_device
        )
        with self.assertRaises(TypeError):
            ht.average(random_5d, weights=random_weights, axis=axis)
        random_shape_weights = ht.random.randn(random_5d.gshape[axis] + 1, device=heat_device)
        with self.assertRaises(ValueError):
            ht.average(random_5d, weights=random_shape_weights, axis=axis)
        zero_weights = ht.zeros((random_5d.gshape[axis]), split=0, device=heat_device)
        with self.assertRaises(ZeroDivisionError):
            ht.average(random_5d, weights=zero_weights, axis=axis)
        weights_5d_split_mismatch = ht.ones(random_5d.gshape, split=-1, device=heat_device)
        with self.assertRaises(NotImplementedError):
    ...
    def test_max(self):
        ...
        ht_array = ht.array(data, device=heat_device)
        comparison = torch.tensor(data, device=torch_device)
        ...
        random_volume = ht.random.randn(3, 3 * size, 3, split=1, device=heat_device)
        ...
        random_volume = ht.random.randn(3 * size, 3, 3, split=0, device=heat_device)
        ...
        random_5d = ht.random.randn(1 * size, 2, 3, 4, 5, split=0, device=heat_device)
        ...
        if size > 1:
            a = ht.arange(size - 1, split=0, device=heat_device)
            res = ht.max(a)
            expected = torch.tensor([size - 2], dtype=a.dtype.torch_type(), device=torch_device)
    ...
    def test_maximum(self):
        ...
        ht_array1 = ht.array(data1, device=heat_device)
        ht_array2 = ht.array(data2, device=heat_device)
        comparison1 = torch.tensor(data1, device=torch_device)
        comparison2 = torch.tensor(data2, device=torch_device)
        ...
        size = ht.MPI_WORLD.size
        torch.manual_seed(1)
        random_volume_1 = ht.random.randn(12 * size, 3, 3, split=0, device=heat_device)
        random_volume_2 = ht.random.randn(12 * size, 1, 3, split=0, device=heat_device)
        ...
        random_volume_1_splitdiff = ht.random.randn(
            size * 3, size * 3, 4, split=0, device=heat_device
        )
        random_volume_2_splitdiff = ht.random.randn(
            size * 3, size * 3, 4, split=1, device=heat_device
        )
        ...
        random_volume_1_split_none = ht.random.randn(
            size * 3, size * 3, 4, split=None, device=heat_device
        )
        random_volume_2_splitdiff = ht.random.randn(
            size * 3, size * 3, 4, split=1, device=heat_device
        )
        ...
        output = ht.empty(out_shape, device=heat_device)
        ...
        # check exceptions
        random_volume_3 = ht.random.randn(4, 2, 3, split=0, device=heat_device)
        with self.assertRaises(ValueError):
            ht.maximum(random_volume_1, random_volume_3)
        random_volume_3 = torch.ones(12, 3, 3, device=torch_device)
        with self.assertRaises(TypeError):
            ht.maximum(random_volume_1, random_volume_3)
        output = torch.ones(12, 3, 3, device=torch_device)
        with self.assertRaises(TypeError):
            ht.maximum(random_volume_1, random_volume_2, out=output)
        output = ht.ones((12, 4, 3), device=heat_device)
        with self.assertRaises(ValueError):
            ht.maximum(random_volume_1, random_volume_2, out=output)
    ...
    def test_mean(self):
        ...
        x = ht.zeros((2, 3, 4), device=heat_device)
        with self.assertRaises(ValueError):
            x.mean(axis=10)
        with self.assertRaises(ValueError):
            x.mean(axis=[4])
        ...
        a = ht.arange(1, 5, device=heat_device)
        ...
        for d in [array_0_len, array_1_len, array_2_len]:
            ...
            for split in hold:  # loop over the number of split dimension of the test array
                z = ht.ones(dimensions, split=split, device=heat_device)
                res = z.mean()
        ...
        ax0 = ht.array(
            [5.84333333333333, 3.054, 3.75866666666667, 1.19866666666667], device=heat_device
        )
        for sp in [None, 0, 1]:
            iris = ht.load("heat/datasets/data/iris.csv", sep=";", split=sp, device=heat_device)
    ...
    def test_min(self):
        ...
        # maximum along first axis
        ht_array = ht.array(data, dtype=ht.int8, device=heat_device)
        ...
        # maximum along second axis
        ht_array = ht.array(data, dtype=ht.int16, device=heat_device)
        ...
        if size > 1:
            a = ht.arange(size - 1, split=0, device=heat_device)
            res = ht.min(a)
            expected = torch.tensor([0], dtype=a.dtype.torch_type(), device=torch_device)
            self.assertTrue(torch.equal(res._DNDarray__array, expected))
