...
class TestExponential(TestCase):
    def test_exp(self):
        ...
        comparison = ht.array(tmp, device=self.ht_device)

        # exponential of float32
        float32_tensor = ht.arange(elements, dtype=ht.float32, device=self.ht_device)
        ...
        float64_tensor = ht.arange(elements, dtype=ht.float64, device=self.ht_device)
        ...
        int32_tensor = ht.arange(elements, dtype=ht.int32, device=self.ht_device)
        ...
        int64_tensor = ht.arange(elements, dtype=ht.int64, device=self.ht_device)
        ...
        # Tests with split
        expected = torch.arange(10, dtype=torch.float32, device=self.torch_device).exp()
        actual = ht.arange(10, split=0, dtype=ht.float32, device=self.ht_device).exp()
    ...
    def test_expm1(self):
        elements = 10
        tmp = torch.arange(elements, dtype=torch.float64, device=self.torch_device).expm1()
        comparison = ht.array(tmp, device=self.ht_device)
    ...
    def test_log(self):
        elements = 15
        tmp = torch.arange(1, elements, dtype=torch.float64, device=self.torch_device).log()
        comparison = ht.array(tmp, device=self.ht_device)

        # logarithm of float32
        float32_tensor = ht.arange(1, elements, dtype=ht.float32, device=self.ht_device)
        ...
        float64_tensor = ht.arange(1, elements, dtype=ht.float64, device=self.ht_device)
        ...
        int32_tensor = ht.arange(1, elements, dtype=ht.int32, device=self.ht_device)
        ...
        int64_tensor = ht.arange(1, elements, dtype=ht.int64, device=self.ht_device)
    ...
    def test_log2(self):
        elements = 15
        tmp = torch.arange(1, elements, dtype=torch.float64, device=self.torch_device).log2()
        comparison = ht.array(tmp, device=self.ht_device)
    ...
    def test_log10(self):
        elements = 15
        tmp = torch.arange(1, elements, dtype=torch.float64, device=self.torch_device).log10()
        comparison = ht.array(tmp, device=self.ht_device)
    ...
    def test_log1p(self):
        elements = 15
        tmp = torch.arange(1, elements, dtype=torch.float64, device=self.torch_device).log1p()
        comparison = ht.array(tmp, device=self.ht_device)
    ...
    def test_sqrt(self):
        elements = 25
        tmp = torch.arange(elements, dtype=torch.float64, device=self.torch_device).sqrt()
        comparison = ht.array(tmp, device=self.ht_device)
    ...
    def test_sqrt_method(self):
        ...
        # square roots of float32
        float32_sqrt = ht.arange(elements, dtype=ht.float32, device=self.ht_device).sqrt()
        self.assertIsInstance(float32_sqrt, ht.DNDarray)
        self.assertEqual(float32_sqrt.dtype, ht.float32)
        self.assertEqual(float32_sqrt.dtype, ht.float32)
        ...
        # square roots of float64
        float64_sqrt = ht.arange(elements, dtype=ht.float64, device=self.ht_device).sqrt()
        self.assertIsInstance(float64_sqrt, ht.DNDarray)
        self.assertEqual(float64_sqrt.dtype, ht.float64)
        self.assertEqual(float64_sqrt.dtype, ht.float64)
        ...
        # square roots of ints, automatic conversion to intermediate floats
        int32_sqrt = ht.arange(elements, dtype=ht.int32, device=self.ht_device).sqrt()
        self.assertIsInstance(int32_sqrt, ht.DNDarray)
        self.assertEqual(int32_sqrt.dtype, ht.float64)
        self.assertEqual(int32_sqrt.dtype, ht.float64)
        ...
        # square roots of longs, automatic conversion to intermediate floats
        int64_sqrt = ht.arange(elements, dtype=ht.int64, device=self.ht_device).sqrt()
        self.assertIsInstance(int64_sqrt, ht.DNDarray)
        self.assertEqual(int64_sqrt.dtype, ht.float64)
        self.assertEqual(int64_sqrt.dtype, ht.float64)
    ...
    def test_sqrt_out_of_place(self):
        ...
        number_range = ht.arange(elements, dtype=ht.float32, device=self.ht_device)
        output_buffer = ht.zeros(output_shape, dtype=ht.float32, device=self.ht_device)

        # square roots
        float32_sqrt = ht.sqrt(number_range, out=output_buffer)
        comparison = torch.arange(elements, dtype=torch.float32, device=self.torch_device).sqrt()
