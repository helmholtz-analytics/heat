...
class TestLogical(unittest.TestCase):
    def test_all(self):
        ...
        ones_noaxis = ht.ones(array_len, device=heat_device)
        x = (ones_noaxis == 1).all()
        ...
        ones_noaxis_split = ht.ones(array_len, split=0, device=heat_device)
        ...
        ones_noaxis_int = ht.ones(array_len, device=heat_device).astype(ht.int)
        ...
        ones_noaxis_split_int = ht.ones(array_len, split=0, device=heat_device).astype(ht.int)
        ...
        ones_noaxis_volume = ht.ones((3, 3, 3), device=heat_device)
        ...
        sequence = ht.arange(array_len, device=heat_device)
        ...
        ones_noaxis_split_axis = ht.ones((3, 3, 3), split=0, device=heat_device)
        ...
        ones_noaxis_split_axis_neg = ht.zeros((1, 2, 3, 4, 5), split=1, device=heat_device)
        ...
        with self.assertRaises(ValueError):
            ht.ones(array_len, device=heat_device).all(axis=1)
        with self.assertRaises(ValueError):
            ht.ones(array_len, device=heat_device).all(axis=-2)
        with self.assertRaises(ValueError):
            ht.ones((4, 4), device=heat_device).all(axis=0, out=out_noaxis)
        with self.assertRaises(TypeError):
            ht.ones(array_len, device=heat_device).all(axis="bad_axis_type")

    def test_allclose(self):
        a = ht.float32([[2, 2], [2, 2]], device=heat_device)
        b = ht.float32([[2.00005, 2.00005], [2.00005, 2.00005]], device=heat_device)
        c = ht.zeros((4, 6), split=0, device=heat_device)
        d = ht.zeros((4, 6), split=1, device=heat_device)
        e = ht.zeros((4, 6), device=heat_device)
    ...
    def test_any(self):
        ...
        x = ht.float32([[2.7, 0, 0], [0, 0, 0], [0, 0.3, 0]], device=heat_device)
        any_tensor = x.any(axis=1)
        res = ht.uint8([1, 0, 1], device=heat_device)
        ...
        any_tensor = ht.zeros((2,), device=heat_device)
        x = ht.int32([[0, 0], [0, 0], [0, 1]], device=heat_device)
        ht.any(x, axis=0, out=any_tensor)
        res = ht.uint8([0, 1], device=heat_device)
        self.assertIsInstance(any_tensor, ht.DNDarray)
        ...
        x = ht.float64([[0, 0, 0], [0, 0, 0]], device=heat_device)
        res = ht.zeros(1, dtype=ht.uint8, device=heat_device)
        any_tensor = ht.any(x)
        self.assertIsInstance(any_tensor, ht.DNDarray)
        ...
        x = ht.arange(10, split=0, device=heat_device)
        any_tensor = ht.any(x, axis=0)
        res = ht.uint8([1], device=heat_device)
        self.assertIsInstance(any_tensor, ht.DNDarray)
        self.assertEqual(any_tensor.shape, (1,))
        self.assertEqual(any_tensor.dtype, ht.bool)
        self.assertTrue(ht.equal(any_tensor, res))
    ...
    def test_isclose(self):
        ...
        a = ht.float32([[2, 2], [2, 2]], device=heat_device)
        b = ht.float32([[2.00005, 2.00005], [2.00005, 2.00005]], device=heat_device)
        c = ht.zeros((4 * size, 6), split=0, device=heat_device)
        d = ht.zeros((4 * size, 6), split=1, device=heat_device)
        e = ht.zeros((4 * size, 6), device=heat_device)
