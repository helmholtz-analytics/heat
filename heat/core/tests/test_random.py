...
class TestRandom(TestCase):
    def test_rand(self):
        ...
        seed = 12345
        ht.random.seed(seed)
        a = ht.random.rand(2, 5, 7, 3, split=0, device=self.ht_device)
        self.assertEqual(a.dtype, ht.float32)
        self.assertEqual(a._DNDarray__array.dtype, torch.float32)
        b = ht.random.rand(2, 5, 7, 3, split=0, device=self.ht_device)
        self.assertFalse(ht.equal(a, b))
        ht.random.seed(seed)
        c = ht.random.rand(2, 5, 7, 3, dtype=ht.float32, split=0, device=self.ht_device)
        self.assertTrue(ht.equal(a, c))

        # Random numbers with overflow
        ht.random.set_state(("Threefry", seed, 0xFFFFFFFFFFFFFFF0))
        a = ht.random.rand(2, 3, 4, 5, split=0, device=self.ht_device)
        ht.random.set_state(("Threefry", seed, 0x10000000000000000))
        b = ht.random.rand(2, 44, split=0, device=self.ht_device)
        a = a.numpy().flatten()
        b = b.numpy().flatten()
        ...
        a = ht.random.rand(2, 44, device=self.ht_device)
        ht.random.seed(seed)
        b = ht.random.rand(2, 44, device=self.ht_device)
        self.assertFalse(ht.equal(a, b))

        # Check that we start from beginning after 128 bit overflow
        ht.random.seed(seed)
        a = ht.random.rand(2, 34, split=0, device=self.ht_device)
        ht.random.set_state(("Threefry", seed, 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0))
        b = ht.random.rand(2, 50, split=0, device=self.ht_device)
        a = a.numpy().flatten()
        b = b.numpy().flatten()
        ...
        ht.random.seed(seed)
        a = ht.random.rand(3, 5, 2, 9, split=3, device=self.ht_device)
        ht.random.seed(seed)
        c = ht.random.rand(3, 5, 2, 9, split=3, device=self.ht_device)
        self.assertTrue(ht.equal(a, c))

        # Random values are in correct order
        ht.random.seed(seed)
        a = ht.random.rand(2, 50, split=0, device=self.ht_device)
        ht.random.seed(seed)
        b = ht.random.rand(100, split=None, device=self.ht_device)
        a = a.numpy().flatten()
        ...
        a = ht.random.rand(3, 5, 2, 9, split=3, device=self.ht_device)
        ht.random.seed(seed)
        b = ht.random.rand(30, 9, split=1, device=self.ht_device)
        a = np.sort(a.numpy().flatten())
        b = np.sort(b.numpy().flatten())
        self.assertTrue(np.array_equal(a, b))

        # One large array does not have two similar values
        a = ht.random.rand(11, 15, 3, 7, split=2, device=self.ht_device)
        a = a.numpy()
        ...
        ht.random.seed(seed)
        a = ht.random.rand(device=self.ht_device)
        ht.random.seed(seed)
        b = ht.random.rand(1, device=self.ht_device)
        self.assertTrue(ht.equal(a, b))

        # To big arrays cant be created
        with self.assertRaises(ValueError):
            ht.random.randn(0xFFFFFFFFFFFFFFFF * 2 + 1, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.random.rand(3, 2, -2, 5, split=1, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.random.randn(12, 43, dtype=ht.int32, split=0, device=self.ht_device)

        # 32 Bit tests
        ht.random.seed(9876)
        shape = (13, 43, 13, 23)
        a = ht.random.rand(*shape, dtype=ht.float32, split=0, device=self.ht_device)
        self.assertEqual(a.dtype, ht.float32)
        self.assertEqual(a._DNDarray__array.dtype, torch.float32)

        ht.random.seed(9876)
        b = ht.random.rand(np.prod(shape), dtype=ht.float32, device=self.ht_device)
        a = a.numpy().flatten()
        b = b._DNDarray__array.cpu().numpy()
        self.assertTrue(np.array_equal(a, b))
        self.assertEqual(a.dtype, np.float32)

        a = ht.random.rand(21, 16, 17, 21, dtype=ht.float32, split=2, device=self.ht_device)
        b = ht.random.rand(15, 11, 19, 31, dtype=ht.float32, split=0, device=self.ht_device)
        a = a.numpy().flatten()
        b = b.numpy().flatten()
        ...
        a = ht.random.rand(12, 32, 44, split=1, dtype=ht.float32, device=self.ht_device).numpy()
        # Overflow reached
        ht.random.set_state(("Threefry", 11111, 0x10000000000000000))
        b = ht.random.rand(12, 32, 44, split=1, dtype=ht.float32, device=self.ht_device).numpy()
        self.assertTrue(np.array_equal(a, b))

        ht.random.set_state(("Threefry", 11111, 0x100000000))
        c = ht.random.rand(12, 32, 44, split=1, dtype=ht.float32, device=self.ht_device).numpy()
        self.assertFalse(np.array_equal(a, c))
        self.assertFalse(np.array_equal(b, c))

    def test_randint(self):
        # Checked that the random values are in the correct range
        a = ht.random.randint(low=0, high=10, size=(10, 10), dtype=ht.int64, device=self.ht_device)
        ...
        a = ht.random.randint(1, size=(10,), split=0, dtype=ht.int64, device=self.ht_device)
        b = ht.zeros((10,), dtype=ht.int64, split=0, device=self.ht_device)
        self.assertTrue(ht.equal(a, b))

        # Two arrays with the same seed and same number of elements have the same random values
        ht.random.seed(13579)
        shape = (15, 13, 9, 21, 65)
        a = ht.random.randint(15, 100, size=shape, split=0, dtype=ht.int64, device=self.ht_device)
        a = a.numpy().flatten()

        ht.random.seed(13579)
        elements = np.prod(shape)
        b = ht.random.randint(
            low=15, high=100, size=(elements,), dtype=ht.int64, device=self.ht_device
        )
        b = b.numpy()
        self.assertTrue(np.array_equal(a, b))
        ...
        ht.random.seed(13579)
        a = ht.random.randint(10000, size=shape, split=2, dtype=ht.int64, device=self.ht_device)
        a = a.numpy()

        ht.random.seed(13579)
        b = ht.random.randint(
            low=0, high=10000, size=shape, split=2, dtype=ht.int64, device=self.ht_device
        )
        b = b.numpy()

        ht.random.seed(13579)
        c = ht.random.randint(low=0, high=10000, dtype=ht.int64, device=self.ht_device)
        ...
        with self.assertRaises(ValueError):
            ht.random.randint(5, 5, size=(10, 10), split=0, device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.random.randint(low=0, high=10, size=(3, -4), device=self.ht_device)
        with self.assertRaises(ValueError):
            ht.random.randint(low=0, high=10, size=(15,), dtype=ht.float32, device=self.ht_device)

        # int32 tests
        ht.random.seed(4545)
        a = ht.random.randint(
            50, 1000, size=(13, 45), dtype=ht.int32, split=0, device=self.ht_device
        )
    ...
    def test_randn(self):
        ...
        a = ht.random.randn(*shape, split=0, dtype=ht.float64, device=self.ht_device)
        ...
        ht.random.seed(54321)
        elements = np.prod(shape)
        b = ht.random.randn(elements, split=0, dtype=ht.float64, device=self.ht_device)
        b = b.numpy()
        a = a.flatten()
        self.assertTrue(np.allclose(a, b))

        # Creating the same array two times without resetting seed results in different elements
        c = ht.random.randn(elements, split=0, dtype=ht.float64, device=self.ht_device)
        c = c.numpy()
        ...
        a = ht.random.randn(*shape, split=5, dtype=ht.float64, device=self.ht_device)
        ht.random.seed(12345)
        b = ht.random.randn(*shape, split=5, dtype=ht.float64, device=self.ht_device)
        self.assertTrue(ht.equal(a, b))
        a = a.numpy()
        b = b.numpy()
        self.assertTrue(np.allclose(a, b))
        ...
        # Tests with float32
        ht.random.seed(54321)
        a = ht.random.randn(30, 30, 30, dtype=ht.float32, split=2, device=self.ht_device)
        self.assertEqual(a.dtype, ht.float32)
        ...
        b = ht.random.randn(30, 30, 30, dtype=ht.float32, split=2, device=self.ht_device).numpy()
        self.assertTrue(np.allclose(a, b))

        c = ht.random.randn(30, 30, 30, dtype=ht.float32, split=2, device=self.ht_device).numpy()
