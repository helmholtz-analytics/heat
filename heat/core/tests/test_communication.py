...
class TestCommunication(BasicTest):
    ...
    def setUpClass(cls):
        ...
        cls.data = torch.tensor(
            [[3, 2, 1], [4, 5, 6]], dtype=torch.float32, device=cls.torch_device
        )
    ...
    def test_contiguous_memory_buffer(self):
        # vector heat tensor
        vector_data = ht.arange(1, 10, device=self.ht_device)
        vector_out = ht.zeros_like(vector_data, device=self.ht_device)
        ...
        tensor_data = torch.arange(3 * 4 * 5 * 6, device=self.torch_device).reshape(3, 4, 5, 6) + 1
        tensor_out = torch.zeros_like(tensor_data, device=self.torch_device)
    ...
    def test_non_contiguous_memory_buffer(self):
        ...
        non_contiguous_data = ht.ones((3, 2), device=self.ht_device).T
        contiguous_out = ht.zeros_like(non_contiguous_data, device=self.ht_device)
        ...
        contiguous_data = ht.ones((3, 2), device=self.ht_device)
        non_contiguous_out = ht.zeros((2, 3), device=self.ht_device).T
        ...
        # non-contiguous destination
        both_non_contiguous_data = ht.ones((3, 2), device=self.ht_device).T
        both_non_contiguous_out = ht.zeros((3, 2), device=self.ht_device).T
    ...
    def test_default_comm(self):
        ...
        a = ht.zeros((4, 5), device=self.ht_device)
        ...
        b = ht.zeros((4, 5), device=self.ht_device)
    ...
    def test_allgather(self):
        # contiguous data
        data = ht.ones((1, 7), device=self.ht_device)
        output = ht.zeros((ht.MPI_WORLD.size, 7), device=self.ht_device)
        ...
        self.assertTrue(
            (
                ...
                == torch.ones(ht.MPI_WORLD.size, 7, device=self.torch_device)
            ).all()
        )

        # contiguous data, different gather axis
        data = ht.ones((7, 2), dtype=ht.float64, device=self.ht_device)
        output = ht.random.randn(7, 2 * ht.MPI_WORLD.size, dtype=ht.float64, device=self.ht_device)
        ...
        self.assertTrue(
            (
                output._DNDarray__array
                == torch.ones(7, 2 * ht.MPI_WORLD.size, device=self.torch_device)
            ).all()
        )

        # non-contiguous data
        data = ht.ones((4, 5), device=self.ht_device).T
        output = ht.zeros((5, 4 * ht.MPI_WORLD.size), device=self.ht_device)
        ...
        self.assertTrue(
            (
                output._DNDarray__array
                == torch.ones(5, 4 * ht.MPI_WORLD.size, device=self.torch_device)
            ).all()
        )

        # non-contiguous output, different gather axis
        data = ht.ones((5, 7), device=self.ht_device)
        output = ht.zeros((7 * ht.MPI_WORLD.size, 5), device=self.ht_device).T
        ...
        self.assertTrue(
            (
                output._DNDarray__array
                == torch.ones(5, 7 * ht.MPI_WORLD.size, device=self.torch_device)
            ).all()
        )

        # contiguous data
        data = ht.array([[ht.MPI_WORLD.rank] * 10], device=self.ht_device)
        output = ht.array([[0] * 10] * ht.MPI_WORLD.size, device=self.ht_device)
        ...
        # check  result
        result = ht.array([np.arange(0, ht.MPI_WORLD.size)] * 10, device=self.ht_device).T
        self.assertTrue(ht.equal(output, result))

        # contiguous data
        data = ht.array([[ht.MPI_WORLD.rank]] * 10, device=self.ht_device)
        output = ht.array([[0] * ht.MPI_WORLD.size] * 10, device=self.ht_device)
        ...
        # check  result
        result = ht.array([np.arange(0, ht.MPI_WORLD.size)] * 10, device=self.ht_device)
        self.assertTrue(ht.equal(output, result))

        # other datatypes (send numpy array)
        data = np.array([ht.MPI_WORLD.rank] * 3)
        output = ht.array([[0] * 3] * ht.MPI_WORLD.size, device=self.ht_device)

        # perform the allgather operation
        ht.MPI_WORLD.Allgatherv(data, output)

        # check  result
        result = ht.array([np.arange(0, ht.MPI_WORLD.size)] * 3, device=self.ht_device).T
        self.assertTrue(ht.equal(output, result))

        data = ht.array([ht.MPI_WORLD.rank] * 3, device=self.ht_device)
        ...
        with self.assertRaises(TypeError):
            data = np.array([ht.MPI_WORLD.rank] * 3, device=self.ht_device)
            output = ht.array([[0] * 3 * ht.MPI_WORLD.size], device=self.ht_device)
            ht.MPI_WORLD.Allgatherv(data, output, recv_axis=1)
        with self.assertRaises(TypeError):
            data = ht.array([ht.MPI_WORLD.rank] * 3, device=self.ht_device)
            output = np.array([[0] * 3 * ht.MPI_WORLD.size], device=self.ht_device)
            ht.MPI_WORLD.Allgatherv(data, output, recv_axis=1)

    def test_allgatherv(self):
        # contiguous data buffer, contiguous output buffer
        data = ht.ones((ht.MPI_WORLD.rank + 1, 10), device=self.ht_device)
        output_count = ht.MPI_WORLD.size * (ht.MPI_WORLD.size + 1) // 2
        output = ht.zeros((output_count, 10), device=self.ht_device)
        ...
        self.assertTrue(
            (
                output._DNDarray__array == torch.ones(output_count, 10, device=self.torch_device)
            ).all()
        )

        # non-contiguous data buffer, contiguous output buffer
        data = ht.ones((10, 2 * (ht.MPI_WORLD.rank + 1)), device=self.ht_device).T
        ...
        data = ht.ones((2 * (ht.MPI_WORLD.rank + 1), 10), device=self.ht_device)
        output_count = ht.MPI_WORLD.size * (ht.MPI_WORLD.size + 1)
        output = ht.zeros((10, output_count), device=self.ht_device).T
        ...
        data = ht.array([[ht.MPI_WORLD.rank] * 10] * (ht.MPI_WORLD.size + 1), device=self.ht_device)

        # contiguous output buffer
        output_shape = data.lshape
        output = ht.zeros(output_shape, dtype=ht.int64, device=self.ht_device)

        # Results for comparison
        first_line = ht.array([[0] * 10], device=self.ht_device)
        last_line = ht.array([[ht.MPI_WORLD.size - 1] * 10], device=self.ht_device)
    ...
    def test_allreduce(self):
        # contiguous data
        data = ht.ones((10, 2), dtype=ht.int8, device=self.ht_device)
        out = ht.zeros_like(data, device=self.ht_device)
        ...
        # non-contiguous data
        data = ht.ones((10, 2), dtype=ht.int8, device=self.ht_device).T
        out = ht.zeros_like(data, device=self.ht_device)
        ...
        data = ht.ones((10, 2), dtype=ht.int8, device=self.ht_device)
        out = ht.zeros((2, 10), dtype=ht.int8, device=self.ht_device).T
    ...
    def test_alltoall(self):
        # contiguous data
        data = ht.array([[ht.MPI_WORLD.rank] * 10] * ht.MPI_WORLD.size, device=self.ht_device)
        output = ht.zeros((ht.MPI_WORLD.size, 10), dtype=ht.int64, device=self.ht_device)
        ...
        self.assertTrue(output._DNDarray__array.is_contiguous())
        comparison = (
            torch.arange(ht.MPI_WORLD.size, device=self.torch_device)
            .reshape(-1, 1)
        ...
        output = ht.zeros((10, ht.MPI_WORLD.size), dtype=ht.int64, device=self.ht_device)
        ...
        # non-contiguous data
        data = ht.ones((10, 2 * ht.MPI_WORLD.size), dtype=ht.int64, device=self.ht_device).T
        output = ht.zeros((2 * ht.MPI_WORLD.size, 10), dtype=ht.int64, device=self.ht_device)
        ...
        comparison = torch.ones(
            (2 * ht.MPI_WORLD.size, 10), dtype=torch.int64, device=self.torch_device
        )
        self.assertTrue((output._DNDarray__array == comparison).all())

        # non-contiguous output, different gather axis
        data = ht.ones((10, 2 * ht.MPI_WORLD.size), dtype=ht.int64, device=self.ht_device)
        output = ht.zeros((2 * ht.MPI_WORLD.size, 10), dtype=ht.int64, device=self.ht_device).T
        ...
        self.assertFalse(output._DNDarray__array.is_contiguous())
        comparison = torch.ones(
            (10, 2 * ht.MPI_WORLD.size), dtype=torch.int64, device=self.torch_device
        )
        self.assertTrue((output._DNDarray__array == comparison).all())
    ...
    def test_alltoallv(self):
        ...
        data = ht.array(
            [[ht.MPI_WORLD.rank] * (ht.MPI_WORLD.size + 1)] * 10, device=self.ht_device
        ).T
        send_counts, send_displs, output_shape = data.comm.counts_displs_shape(data.lshape, 0)
        ...
        output = ht.zeros(output_shape, dtype=ht.int64, device=self.ht_device).T
        recv_counts, recv_displs, _ = data.comm.counts_displs_shape(output.lshape, 0)
    ...
    def test_bcast(self):
        # contiguous data
        data = ht.arange(10, dtype=ht.int64, device=self.ht_device)
        if ht.MPI_WORLD.rank != 0:
            data = ht.zeros_like(data, dtype=ht.int64, device=self.ht_device)
        ...
        self.assertTrue(data._DNDarray__array.is_contiguous())
        self.assertTrue((data._DNDarray__array == torch.arange(10, device=self.torch_device)).all())

        # non-contiguous data
        data = ht.ones((2, 5), dtype=ht.float32, device=self.ht_device).T
        if ht.MPI_WORLD.rank != 0:
            data = ht.zeros((2, 5), dtype=ht.float32, device=self.ht_device).T
        ...
        self.assertTrue(
            (
                ...
                == torch.ones((5, 2), dtype=torch.float32, device=self.torch_device)
            ).all()
        )

    def test_exscan(self):
        # contiguous data
        data = ht.ones((5, 3), dtype=ht.int64, device=self.ht_device)
        out = ht.zeros_like(data, device=self.ht_device)
        ...
        # non-contiguous data
        data = ht.ones((5, 3), dtype=ht.int64, device=self.ht_device).T
        out = ht.zeros_like(data, device=self.ht_device)
        ...
        # non-contiguous output
        data = ht.ones((5, 3), dtype=ht.int64, device=self.ht_device)
        out = ht.zeros((3, 5), dtype=ht.int64, device=self.ht_device).T
    ...
    def test_gather(self):
        # contiguous data
        data = ht.ones((1, 5), device=self.ht_device)
        output = ht.zeros((ht.MPI_WORLD.size, 5), device=self.ht_device)
        ...
        if data.comm.rank == 0:
            self.assertTrue(
                (
                    output._DNDarray__array
                    == torch.ones(ht.MPI_WORLD.size, 5, device=self.torch_device)
                ).all()
            )

        # contiguous data, different gather axis
        data = ht.ones((5, 2), device=self.ht_device)
        output = ht.zeros((5, 2 * ht.MPI_WORLD.size), device=self.ht_device)
        ...
        if data.comm.rank == 0:
            self.assertTrue(
                (
                    output._DNDarray__array
                    == torch.ones(5, 2 * ht.MPI_WORLD.size, device=self.torch_device)
                ).all()
            )

        # non-contiguous data
        data = ht.ones((3, 5), device=self.ht_device).T
        output = ht.zeros((5, 3 * ht.MPI_WORLD.size), device=self.ht_device)
        ...
        if data.comm.rank == 0:
            self.assertTrue(
                (
                    output._DNDarray__array
                    == torch.ones(5, 3 * ht.MPI_WORLD.size, device=self.torch_device)
                ).all()
            )

        # non-contiguous output, different gather axis
        data = ht.ones((5, 3), device=self.ht_device)
        output = ht.zeros((3 * ht.MPI_WORLD.size, 5), device=self.ht_device).T
    ...
    def test_gatherv(self):
        ...
        if data.comm.rank == 0:
            self.assertTrue(
                (
                    output._DNDarray__array
                    == torch.ones(output_count, 10, device=self.torch_device)
                ).all()
            )
    ...
    def test_iallgather(self):
        try:
            ...
            # contiguous data, different gather axis
            data = ht.ones((7, 2), dtype=ht.float64, device=self.ht_device)
            output = ht.random.randn(
                7, 2 * ht.MPI_WORLD.size, dtype=ht.float64, device=self.ht_device
            )
    ...
    def test_ialltoall(self):
        try:
            ...
            # contiguous data, different gather axis
            data = ht.array([[ht.MPI_WORLD.rank] * ht.MPI_WORLD.size] * 10, device=self.ht_device)
            output = ht.zeros((10, ht.MPI_WORLD.size), dtype=ht.int64, device=self.ht_device)
    ...
    def test_ialltoallv(self):
        try:
            # contiguous data buffer
            data = ht.array(
                [[ht.MPI_WORLD.rank] * 10] * (ht.MPI_WORLD.size + 1), device=self.ht_device
            )
            send_counts, send_displs, output_shape = data.comm.counts_displs_shape(data.lshape, 0)
    ...
    def test_ibcast(self):
        try:
            ...
            self.assertTrue(data._DNDarray__array.is_contiguous())
            self.assertTrue(
                (data._DNDarray__array == torch.arange(10, device=self.torch_device)).all()
            )
    ...
    def test_igather(self):
        try:
            # contiguous data
            data = ht.ones((1, 5), dtype=ht.float64, device=self.ht_device)
            output = ht.random.randn(ht.MPI_WORLD.size, 5, dtype=ht.float64, device=self.ht_device)
            ...
            if data.comm.rank == 0:
                self.assertTrue(
                    (
                        output._DNDarray__array
                        == torch.ones(
                            (ht.MPI_WORLD.size, 5), dtype=torch.float32, device=self.torch_device
                        )
                    ).all()
                )

            # contiguous data, different gather axis
            data = ht.ones((5, 2), dtype=ht.float64, device=self.ht_device)
            output = ht.random.randn(
                5, 2 * ht.MPI_WORLD.size, dtype=ht.float64, device=self.ht_device
            )
            ...
            # non-contiguous data
            data = ht.ones((3, 5), dtype=ht.float64, device=self.ht_device).T
            output = ht.random.randn(
                5, 3 * ht.MPI_WORLD.size, dtype=ht.float64, device=self.ht_device
            )
            ...
            # non-contiguous output, different gather axis
            data = ht.ones((5, 3), dtype=ht.float64, device=self.ht_device)
            output = ht.random.randn(
                3 * ht.MPI_WORLD.size, 5, dtype=ht.float64, device=self.ht_device
            ).T
    ...
    def test_ireduce(self):
        try:
            # contiguous data
            data = ht.ones((10, 2), dtype=ht.int32, device=self.ht_device)
            out = ht.zeros_like(data, device=self.ht_device)
            ...
            # non-contiguous data
            data = ht.ones((10, 2), dtype=ht.int32, device=self.ht_device).T
            out = ht.zeros_like(data, device=self.ht_device)
            ...
            # non-contiguous output
            data = ht.ones((10, 2), dtype=ht.int32, device=self.ht_device)
            out = ht.zeros((2, 10), dtype=ht.int32, device=self.ht_device).T
    ...
    def test_iscan(self):
        try:
            ...
            # non-contiguous data
            data = ht.ones((5, 3), dtype=ht.float64, device=self.ht_device).T
            out = ht.zeros_like(data, device=self.ht_device)
            ...
            # non-contiguous output
            data = ht.ones((5, 3), dtype=ht.float64, device=self.ht_device)
            out = ht.zeros((3, 5), dtype=ht.float64, device=self.ht_device).T
    ...
    def test_iscatter(self):
        try:
            # contiguous data
            if ht.MPI_WORLD.rank == 0:
                data = ht.ones((ht.MPI_WORLD.size, 5), device=self.ht_device)
            else:
                data = ht.zeros((1,), device=self.ht_device)
            output = ht.zeros((1, 5), device=self.ht_device)
            ...
            self.assertTrue(output._DNDarray__array.is_contiguous())
            self.assertTrue(
                (output._DNDarray__array == torch.ones(1, 5, device=self.torch_device)).all()
            )

            # contiguous data, different scatter axis
            if ht.MPI_WORLD.rank == 0:
                data = ht.ones((5, ht.MPI_WORLD.size), device=self.ht_device)
            else:
                data = ht.zeros((1,), device=self.ht_device)
            output = ht.zeros((5, 1), device=self.ht_device)
            ...
            self.assertTrue(output._DNDarray__array.is_contiguous())
            self.assertTrue(
                (output._DNDarray__array == torch.ones(5, 1, device=self.torch_device)).all()
            )

            # non-contiguous data
            if ht.MPI_WORLD.rank == 0:
                data = ht.ones((5, ht.MPI_WORLD.size * 2), device=self.ht_device).T
                self.assertFalse(data._DNDarray__array.is_contiguous())
            else:
                data = ht.zeros((1,), device=self.ht_device)
                self.assertTrue(data._DNDarray__array.is_contiguous())
            output = ht.zeros((2, 5), device=self.ht_device)
            ...
            self.assertTrue(output._DNDarray__array.is_contiguous())
            self.assertTrue(
                (output._DNDarray__array == torch.ones(2, 5, device=self.torch_device)).all()
            )

            # non-contiguous destination, different split axis
            if ht.MPI_WORLD.rank == 0:
                data = ht.ones((5, ht.MPI_WORLD.size * 2), device=self.ht_device)
            else:
                data = ht.zeros((1,), device=self.ht_device)
            output = ht.zeros((2, 5), device=self.ht_device).T
            ...
            self.assertFalse(output._DNDarray__array.is_contiguous())
            self.assertTrue(
                (output._DNDarray__array == torch.ones(5, 2, device=self.torch_device)).all()
            )
    ...
    def test_iscatterv(self):
        try:
            ...
            data = ht.ones((input_count, 12), device=self.ht_device)
            output_count = 2 * (ht.MPI_WORLD.rank + 1)
            output = ht.zeros((output_count, 12), device=self.ht_device)
            ...
            self.assertTrue(
                (
                    output._DNDarray__array
                    == torch.ones(output_count, 12, device=self.torch_device)
                ).all()
            )

            # non-contiguous data buffer, contiguous output buffer
            input_count = ht.MPI_WORLD.size * (ht.MPI_WORLD.size + 1)
            data = ht.ones((12, input_count), device=self.ht_device).T
            output_count = 2 * (ht.MPI_WORLD.rank + 1)
            output = ht.zeros((output_count, 12), device=self.ht_device)
            ...
            data = ht.ones((input_count, 12), device=self.ht_device)
            output_count = 2 * (ht.MPI_WORLD.rank + 1)
            output = ht.zeros((12, output_count), device=self.ht_device).T
    ...
    def test_mpi_in_place(self):
        ...
        data = ht.ones((size, size), dtype=ht.int32, device=self.ht_device)
    ...
    def test_scatter_like_axes(self):
        ...
        data = ht.array(
            [[ht.MPI_WORLD.rank] * ht.MPI_WORLD.size] * ht.MPI_WORLD.size, device=self.ht_device
        )
        output = ht.zeros_like(data, device=self.ht_device)
        ...
        data = ht.array(
            [[ht.MPI_WORLD.rank] * (2 * ht.MPI_WORLD.size)] * ht.MPI_WORLD.size,
            device=self.ht_device,
        )
        output = ht.zeros(
            (2 * ht.MPI_WORLD.size, ht.MPI_WORLD.size), dtype=data.dtype, device=self.ht_device
        )
        ...
        data = ht.array([range(ht.MPI_WORLD.size)] * ht.MPI_WORLD.size, device=self.ht_device)
        output = ht.zeros(
            (ht.MPI_WORLD.size, ht.MPI_WORLD.size), dtype=data.dtype, device=self.ht_device
        )
        data.comm.Alltoall(data, output, send_axis=0, recv_axis=1)
        comparison = (
            torch.arange(ht.MPI_WORLD.size, device=self.torch_device)
    ...
    def test_scatterv(self):
        ...
        self.assertTrue(
            (
                output._DNDarray__array == torch.ones(output_count, 12, device=self.torch_device)
            ).all()
        )
    ...
    def test_allgathervSorting(self):
        ...
        gathered1 = torch.empty(self.sorted3Dtensor.shape, device=self.torch_device)
        gathered2_counts, gathered2_displs, _ = test2.comm.counts_displs_shape(
            test2.shape, test2.split
        )
        gathered2 = torch.empty(self.sorted3Dtensor.shape, device=self.torch_device)
        gathered3_counts, gathered3_displs, _ = test3.comm.counts_displs_shape(
            test3.shape, test3.split
        )
        gathered3 = torch.empty(self.sorted3Dtensor.shape, device=self.torch_device)
    ...
    def test_alltoallSorting(self):
        ...
        redistributed1 = torch.empty(
            comparison1.lshape, dtype=test1.dtype.torch_type(), device=self.torch_device
        )
        ...
        redistributed2 = torch.empty(
            comparison2.lshape, dtype=test2.dtype.torch_type(), device=self.torch_device
        )
        ...
        redistributed3 = torch.empty(
            comparison3.lshape, dtype=test3.dtype.torch_type(), device=self.torch_device
        )
        ...
        redistributed4 = torch.empty(
            comparison4.lshape, dtype=test4.dtype.torch_type(), device=self.torch_device
        )
