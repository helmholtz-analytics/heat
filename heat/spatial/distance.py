...
def _dist(X: DNDarray, Y: DNDarray = None, metric: Callable = _euclidian) -> DNDarray:
    ...
    if Y is None:
        ...
        if X.split is None:
            d.V_local_larray = metric(X.V_local_larray, X.V_local_larray)
            ...
            for iter in range(1, num_iter):
                ...
                d.V_local_larray[:, columns[0] : columns[1]] = d_ij
                ...
                if (rank // iter) == 0:
                    comm.Recv(symmetric, source=receiver, tag=iter)
                d.V_local_larray[:, scolumns[0] : scolumns[1]] = symmetric.transpose(0, 1)
        ...
        if X.split is None:
            d.V_local_larray = metric(X.V_local_larray, Y.V_local_larray)
            ...
            if Y.split is None:
                ...
                d.V_local_larray[:, cols[0] : cols[1]] = d_ij
