...
def main():
    ...
    # change the learning rate adjustments to be reduce on plateau
    args.lr = 0.0125
    # old LR (1. / args.world_size * (5 * (args.world_size - 1) / 6.)) * 0.0125 * args.world_size
    ...
    # todo: fix scheduler to do the steps
    ...
    # import pandas as pd
    comm = htmodel.comm
    nodes = str(int(comm.size / torch.cuda.device_count()))
    cwd = os.getcwd()
    fname = (
        cwd + "/" + nodes + f"imagenet-benchmark-n-{nodes}-skips-{args.batch_skip}-btw-{args.gs}"
    )
    if args.resume and rank == 0 and os.path.isfile(fname + ".pkl"):
        with open(fname + ".pkl", "rb") as f:
            out_dict = pickle.load(f)
        nodes2 = str(comm.size / torch.cuda.device_count())
        old_keys = [
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        scheduler.step(ls)
        print0("next lr:", dp_optimizer.torch_optimizer.param_groups[0]["lr"])
    ...
    if args.rank == 0:
        ...
        if args.benchmarking:
            try:
                fulldf = pd.read_csv(
                    cwd
                    + f"/bench-results-n-{args.world_size / 4}-skips-{args.batch_skip}-btw-{args.gs}.csv"
                )
...
def save_checkpoint(state, is_best):
    ...
    filename = "imgnet-classic-checkpoint-" + str(sz) + ".pth.tar"
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, "model_best-classic.pth.tar")
