...
class Model_Sup(ht.nn.Module):
    def __init__(self, feature_dim=128, num_class=10):
        ...
        for name, module in resnet50().named_children():
            if name == 'conv1':
                module = ht.nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
            if not isinstance(module, nn.Linear) and not isinstance(module, ht.nn.MaxPool2d):
                self.f.append(module)
        # encoder
        self.f = ht.nn.Sequential(*self.f)
        # projection head
        self.g = ht.nn.Sequential(ht.nn.Linear(2048, 512, bias=False), ht.nn.BatchNorm1d(512),
                               ht.nn.ReLU(inplace=True), ht.nn.Linear(512, feature_dim, bias=True))

        # classification head
        self.c = ht.nn.Linear(2048, num_class, bias=True)
