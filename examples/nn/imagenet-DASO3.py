...
def main():
    ...
    # the learning rate adjustments to be reduce on plateau
    args.lr = 0.001  # 0.0125  # (1. / args.world_size * (5 * (args.world_size - 1) / 6.)) * 0.0125 * args.world_size
    ...
    if args.resume:
        ...
        # load the model state (global call) and the local optimizer (optimizer)
        resume(model=htmodel, optimizer=optimizer)
