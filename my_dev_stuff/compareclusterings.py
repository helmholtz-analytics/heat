...
data = ht.load("/projects/HPDAGrundlagensoftware-Heat/Testdata/JPL_SBDB/sbdb_asteroids.h5", load_fraction=0.1, dataset='data', split=0, dtype=ht.float32, device="cpu")
    ...
    for k in clusternumbers:
        ...
        for s in range(n_samples):
            if ht.MPI_WORLD.rank == 0:
                print(f'\t Sample {s}')
            if todo is "bpkm2":
                clusterer = ht.cluster.BatchParallelKMeans(n_clusters=k,init="k-means++",n_procs_to_merge=2)
                titlestr = "BatchParallelKMeans (merge 2)" if ht.MPI_WORLD.size > 1 else "KMeans"
            elif todo is "bpkm":
                clusterer = ht.cluster.BatchParallelKMeans(n_clusters=k,init="k-means++",n_procs_to_merge=None)
                titlestr = "BatchParallelKMeans (all-at-once)" if ht.MPI_WORLD.size > 1 else "KMeans"
            else:
